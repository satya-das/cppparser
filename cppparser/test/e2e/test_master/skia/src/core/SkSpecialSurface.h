...
#  if  SK_SUPPORT_GPU
     ...
     */
  static sk_sp<SkSpecialSurface> MakeRenderTarget(GrRecordingContext*, int width, int height, GrColorType, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* = nullptr);
#  endif
    /**
     * Use and existing SkBitmap as the backing store.
     */
  static sk_sp<SkSpecialSurface> MakeFromBitmap(const SkIRect& subset, SkBitmap& bm, const SkSurfaceProps* = nullptr);
    /**
