...
namespace skvm
{
 ...
 None, ARMDisp19, X86Disp32
    ...
    void vbroadcastss(Ymm dst, GP64 ptr, int off);
    void vpshufb(Ymm dst, Ymm x, Label*);
    void vmovups(Ymm dst, GP64 ptr);
    void vpmovzxwd(Ymm dst, GP64 ptr);
    void vpmovzxbd(Ymm dst, GP64 ptr);
    void vmovd(Xmm dst, GP64 ptr);
    void vmovups(GP64 ptr, Ymm src);
    void vmovups(GP64 ptr, Xmm src);
    void vmovq(GP64 ptr, Xmm src);
    void vmovd(GP64 ptr, Xmm src);
    void movzbl(GP64 dst, GP64 ptr, int off);
    void movb(GP64 ptr, GP64 src);
    void vmovd_direct(GP64 dst, Xmm src);
    void vmovd_direct(Xmm dst, GP64 src);
    void vpinsrw(Xmm dst, Xmm src, GP64 ptr, int imm);
    void vpinsrb(Xmm dst, Xmm src, GP64 ptr, int imm);
    void vpextrw(GP64 ptr, Xmm src, int imm);
    void vpextrb(GP64 ptr, Xmm src, int imm);
        // aarch64
    ...
    void ldrq(V dst, Label*);
    void ldrq(V dst, X src);
    void ldrs(V dst, X src);
    void ldrb(V dst, X src);
    void strq(V src, X dst);
    void strs(V src, X dst);
    void strb(V src, X dst);
  private:
        // dst = op(dst, imm)
 ...
 eq,ne,cs,cc,mi,pl,vs,vc,hi,ls,ge,lt,gt,le,al
  ...
  public:
    ...
    I32 bit_clear(I32 x, I32 y);
    I32 select(I32 cond, I32 t, I32 f);
    ...
    I32 extract(I32 x, int bits, I32 y);
  ...
  private:
    struct InstructionHash
    {
      ...
      {
        return Hash((uint8_t) inst.op) ^ Hash(inst.x) ^ Hash(inst.y) ^ Hash(inst.z) ^ Hash(inst.imm) ^ Hash(inst.death) ^ Hash(inst.hoist);
      }
    };
  ...
  public:
    ...
    Program(const std::vector<Builder::Instruction>& instructions, const std::vector<int>& strides, const char* debug_name);
    Program();
    ~Program();
    Program(Program&&);
