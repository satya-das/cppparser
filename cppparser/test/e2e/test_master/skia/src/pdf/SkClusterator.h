...
public:
  ...
  struct Cluster
  {
    ...
    {
      return fUtf8Text == o.fUtf8Text && fTextByteLength == o.fTextByteLength && fGlyphIndex == o.fGlyphIndex && fGlyphCount == o.fGlyphCount;
    }
  };
  Cluster next();
private:
  const uint32_t* const fClusters;
  const char* const fUtf8Text;
  const uint32_t fGlyphCount;
  const uint32_t fTextByteLength;
  const bool fReversedChars;
