...
public:
  GrAppliedHardClip();
  GrAppliedHardClip(GrAppliedHardClip&& that);
  ...
  {
    return fScissorState == that.fScissorState && fWindowRectsState == that.fWindowRectsState && fStencilStackID == that.fStencilStackID;
  }
  bool operator!=(const GrAppliedHardClip& that) const
  {
    return !(*this == that);
...
public:
  ...
  int numClipCoverageFragmentProcessors() const
  {
    return fClipCoverageFPs.count();
  }
