...
public:
  ...
  static std::unique_ptr<GrFragmentProcessor> Make(const sk_sp<GrTextureProxy> proxies[], const SkYUVAIndex indices[4], SkYUVColorSpace yuvColorSpace, GrSamplerState::Filter filterMode, const SkMatrix& localMatrix = SkMatrix::I(), const SkRect* domain = nullptr);
#  ifdef SK_DEBUG
...
private:
    ...
    , fYUVColorSpace(yuvColorSpace)
  {
    for (int i = 0; i < numPlanes; ++i)
    {
      ...
      fSamplers[i].reset(std::move(proxies[i]), GrSamplerState(GrSamplerState::WrapMode::kClamp, filterModes[i]));
      fSamplerTransforms[i] = planeMatrix;
      fSamplerCoordTransforms[i] = GrCoordTransform(fSamplerTransforms[i], fSamplers[i].proxy());
    }
