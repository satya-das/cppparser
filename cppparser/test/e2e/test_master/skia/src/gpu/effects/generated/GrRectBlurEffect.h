...
{
public:
  static sk_sp<GrTextureProxy> CreateIntegralTexture(GrProxyProvider* proxyProvider, float sixSigma)
  {
    ...
    builder[0] = width;
    builder.finish();
    sk_sp<GrTextureProxy> proxy(proxyProvider->findOrCreateProxyByUniqueKey(key, GrColorType::kAlpha_8, kTopLeft_GrSurfaceOrigin));
    if (!proxy)
    {
      SkBitmap bitmap;
  ...
  static std::unique_ptr<GrFragmentProcessor> Make(GrProxyProvider* proxyProvider, const GrShaderCaps& caps, const SkRect& rect, float sigma)
  {
    ...
    if (!caps.floatIs32Bits())
    {
      ...
      if (SkScalarAbs(rect.fLeft) > 16000.f || SkScalarAbs(rect.fTop) > 16000.f || SkScalarAbs(rect.fRight) > 16000.f || SkScalarAbs(rect.fBottom) > 16000.f)
      {
        return nullptr;
      }
    }
    ...
    SkRect insetRect = {rect.fLeft + threeSigma, rect.fTop + threeSigma, rect.fRight - threeSigma, rect.fBottom - threeSigma};
    ...
    return std::unique_ptr<GrFragmentProcessor>(new GrRectBlurEffect(insetRect, std::move(integral), invSixSigma, isFast, GrSamplerState::ClampBilerp()));
  }
  ...
  bool isFast;
private:
  GrRectBlurEffect(SkRect rect, sk_sp<GrTextureProxy> integral, float invSixSigma, bool isFast, GrSamplerState samplerParams)
