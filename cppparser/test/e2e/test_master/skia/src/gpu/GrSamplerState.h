...
public:
 ...
 kClamp, kRepeat, kMirrorRepeat, kClampToBorder
  ...
  {
    return (WrapMode::kClamp != fWrapModes[0] && WrapMode::kClampToBorder != fWrapModes[0]) || (WrapMode::kClamp != fWrapModes[1] && WrapMode::kClampToBorder != fWrapModes[1]);
  }
  bool operator==(const GrSamplerState& that) const
  {
    return fWrapModes[0] == that.fWrapModes[0] && fWrapModes[1] == that.fWrapModes[1] && fFilter == that.fFilter;
  }
  bool operator!=(const GrSamplerState& that) const
  {
    return !(*this == that);
