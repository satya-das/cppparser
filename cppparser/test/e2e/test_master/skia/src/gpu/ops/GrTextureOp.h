 ...
 */
  std::unique_ptr<GrDrawOp> Make(GrRecordingContext*, sk_sp<GrTextureProxy>, sk_sp<GrColorSpaceXform>, GrSamplerState::Filter, const SkPMColor4f&, Saturate, SkBlendMode, GrAAType, GrQuadAAFlags, const GrQuad& deviceQuad, const GrQuad& localQuad, const SkRect* domain = nullptr);
// Unlike the single-proxy factory, this only supports src-over blending.
  std::unique_ptr<GrDrawOp> MakeSet(GrRecordingContext*, const GrRenderTargetContext::TextureSetEntry[], int cnt, GrSamplerState::Filter, Saturate, GrAAType, SkCanvas::SrcRectConstraint, const SkMatrix& viewMatrix, sk_sp<GrColorSpaceXform> textureXform);
}
#endif
