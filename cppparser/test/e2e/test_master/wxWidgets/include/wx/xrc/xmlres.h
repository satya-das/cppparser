...
#    define WX_XMLRES_CURRENT_VERSION(WX_XMLRES_CURRENT_VERSION_MAJOR * 256*256*256 + \
...
#    endif
  ...
  {
    return DoLoadObject(parent, name, classname, false);
  }
    // Load an object from the resource specifying both the resource name and
  ...
  {
    return DoLoadObject(parent, name, classname, true);
  }
  ...
  {
    long diff = GetVersion() - (major * 256 * 256 * 256 + minor * 256 * 256 + release * 256 + revision);
    if (diff < 0)
    {
      return -1;
    }
...
protected:
  ...
  wxXmlNode* GetResourceNodeAndLocation(const wxString& name, const wxString& classname, bool recursive = false, wxString* path = NULL) const;
  ...
  wxXmlNode* DoFindResource(wxXmlNode* parent, const wxString& name, const wxString& classname, bool recursive) const;
...
public:
  ...
  wxObject* CreateResource(wxXmlNode* node, wxObject* parent, wxObject* instance) override;
    // --- Handy methods:
  ...
  wxSize GetSize(const wxString& param = wxT("size"), wxWindow* windowToUse = NULL) override;
    // Gets the position (may be in dialog units).
  wxPoint GetPosition(const wxString& param = wxT("pos")) override;
    // Gets a dimension (may be in dialog units).
  wxCoord GetDimension(const wxString& param, wxCoord defaultv = 0, wxWindow* windowToUse = NULL) override;
  ...
  wxDirection GetDirection(const wxString& param, wxDirection dirDefault = wxLEFT) override;
    // Gets a bitmap.
  wxBitmap GetBitmap(const wxString& param = wxT("bitmap"), const wxArtClient& defaultArtClient = wxASCII_STR(wxART_OTHER), wxSize size = wxDefaultSize) override;
    // Gets a bitmap from an XmlNode.
  wxBitmap GetBitmap(const wxXmlNode* node, const wxArtClient& defaultArtClient = wxASCII_STR(wxART_OTHER), wxSize size = wxDefaultSize) override;
    // Gets an icon.
  wxIcon GetIcon(const wxString& param = wxT("icon"), const wxArtClient& defaultArtClient = wxASCII_STR(wxART_OTHER), wxSize size = wxDefaultSize) override;
    // Gets an icon from an XmlNode.
  wxIcon GetIcon(const wxXmlNode* node, const wxArtClient& defaultArtClient = wxASCII_STR(wxART_OTHER), wxSize size = wxDefaultSize) override;
    // Gets an icon bundle.
  wxIconBundle GetIconBundle(const wxString& param, const wxArtClient& defaultArtClient = wxASCII_STR(wxART_OTHER)) override;
    // Gets an image list.
...
#    if  wxUSE_ANIMATIONCTRL
  ...
  wxAnimation* GetAnimation(const wxString& param = wxT("animation"), wxAnimationCtrlBase* ctrl = NULL) override;
#    endif
    // Gets a font.
  ...
  wxObject* CreateResFromNode(wxXmlNode* node, wxObject* parent, wxObject* instance = NULL) override;
    // helper
#    if  wxUSE_FILESYSTEM
