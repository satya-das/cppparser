...
#    endif
  ...
  {
    return (m_flags & wxPG_PROP_CATEGORY) != 0;
  }
  ...
  {
    return !(m_flags & wxPG_PROP_DISABLED);
  }
...
#    endif
  ...
  wxPropertyGridPageState* GetParentState() const
  {
...
protected:
  ...
  void DoGenerateComposedValue(wxString& text, int argFlags = wxPG_VALUE_IS_CURRENT, const wxVariantList* valueOverrides = NULL, wxPGHashMapS2S* childResults = NULL) const;
  ...
  void InitAfterAdded(wxPropertyGridPageState* pageState, wxPropertyGrid* propgrid);
  ...
  void OnDetached(wxPropertyGridPageState* state, wxPropertyGrid* propgrid);
...
//
#    define WX_PG_DECLARE_DOGETEDITORCLASS	virtual const wxPGEditor* DoGetEditorClass() const wxOVERRIDE;
