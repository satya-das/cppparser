...
#  endif
  ...
  void reserve(size_type n)
  {
    ...
    const size_type increment = m_size > ALLOC_INITIAL_SIZE ? m_size : (size_type) ALLOC_INITIAL_SIZE;
  ...
  iterator insert(iterator it, size_type count, const value_type& v)
  {
    ...
    wxScopeGuard moveBack = wxMakeGuard(Ops::MemmoveBackward, place, place + count, after);
    if (!after)
    {
      moveBack.Dismiss();
    }
