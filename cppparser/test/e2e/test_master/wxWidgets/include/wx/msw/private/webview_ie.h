...
public:
  ...
  HRESULT STDMETHODCALLTYPE CompareUrl(LPCWSTR pwzUrl1, LPCWSTR pwzUrl2, DWORD dwCompareFlags) override;
  HRESULT STDMETHODCALLTYPE QueryInfo(LPCWSTR pwzUrl, wxQUERYOPTION OueryOption, DWORD dwQueryFlags, LPVOID pBuffer, DWORD cbBuffer, DWORD* pcbBuf, DWORD dwReserved) override;
};
...
public:
  ...
  HRESULT wxSTDCALL ShowContextMenu(DWORD dwID, POINT* ppt, IUnknown* pcmdtReserved, IDispatch* pdispReserved) override;
  HRESULT wxSTDCALL GetHostInfo(DOCHOSTUIINFO* pInfo) override;
  HRESULT wxSTDCALL ShowUI(DWORD dwID, IOleInPlaceActiveObject* pActiveObject, IOleCommandTarget* pCommandTarget, IOleInPlaceFrame* pFrame, IOleInPlaceUIWindow* pDoc) override;
  ...
  HRESULT wxSTDCALL ResizeBorder(LPCRECT prcBorder, IOleInPlaceUIWindow* pUIWindow, BOOL fRameWindow) override;
  HRESULT wxSTDCALL TranslateAccelerator(LPMSG lpMsg, const GUID* pguidCmdGroup, DWORD nCmdID) override;
  HRESULT wxSTDCALL GetOptionKeyPath(LPOLESTR* pchKey, DWORD dw) override;
  HRESULT wxSTDCALL GetDropTarget(IDropTarget* pDropTarget, IDropTarget** ppDropTarget) override;
  HRESULT wxSTDCALL GetExternal(IDispatch** ppDispatch) override;
  HRESULT wxSTDCALL TranslateUrl(DWORD dwTranslate, OLECHAR* pchURLIn, OLECHAR** ppchURLOut) override;
  HRESULT wxSTDCALL FilterDataObject(IDataObject* pDO, IDataObject** ppDORet) override;
    //IUnknown
  DECLARE_IUNKNOWN_METHODS;
private:
  wxWebView* m_browser;
