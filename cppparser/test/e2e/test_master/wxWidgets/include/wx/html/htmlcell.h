...
public:
  wxHtmlSelection()
    : m_fromPos(wxDefaultPosition)
    , m_toPos(wxDefaultPosition)
    , m_fromCharacterPos(-1)
    , m_toCharacterPos(-1)
    , m_fromCell(NULL)
    , m_toCell(NULL)
    , m_extBeforeSel(0)
  ...
  bool IsEmpty() const
  {
    return m_fromPos == wxDefaultPosition && m_toPos == wxDefaultPosition;
  }
private:
