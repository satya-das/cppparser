...
#  ifdef __SGI__
     ...
     */
wxEXTERNC size_t _xpg5_wcsftime(wchar_t*, size_t, const wchar_t*, const struct tm*);
...
WXDLLIMPEXP_BASE size_t wxCRT_StrftimeW(wchar_t* s, size_t max, const wchar_t* fmt, const struct tm* tm);
#  endif
/* -------------------------------------------------------------------------
                                ctype.h
   ------------------------------------------------------------------------- */
...
#    undef warning "Custom mb/wchar conv. only works for ASCII, see Android NDK notes"
