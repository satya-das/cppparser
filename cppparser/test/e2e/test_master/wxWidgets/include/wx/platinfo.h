...
public:
  wxPlatformInfo();
  wxPlatformInfo(wxPortId pid, int tkMajor = -1, int tkMinor = -1, wxOperatingSystemId id = wxOS_UNKNOWN, int osMajor = -1, int osMinor = -1, wxBitness bitness = wxBITNESS_INVALID, wxEndianness endian = wxENDIAN_INVALID, bool usingUniversal = false);
...
protected:
  ...
  int m_osVersionMajor, m_osVersionMinor, m_osVersionMicro;
    // Operating system ID.
