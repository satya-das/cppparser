...
WXDLLIMPEXP_BASE extern const char* wxLOG_COMPONENT;
#    endif
// ----------------------------------------------------------------------------
// constants
// ----------------------------------------------------------------------------
...
public:
  ...
  wxLogRecordInfo(const char* filename_, int line_, const char* func_, const char* component_)
  {
    filename = filename_;
    func = func_;
    line = line_;
...
struct wxLogRecord
{
  wxLogRecord(wxLogLevel level_, const wxString& msg_, const wxLogRecordInfo& info_)
    : level(level_)
    , msg(msg_)
    , info(info_)
  {
...
public:
    // ctor
  wxLog()
    : m_formatter(new wxLogFormatter)
  {
  }
...
#    endif
  ...
  static void OnLog(wxLogLevel level, const wxString& msg, const wxLogRecordInfo& info);
...
#    endif
  ...
  void CallDoLogNow(wxLogLevel level, const wxString& msg, const wxLogRecordInfo& info);
    // variables
    // ----------------
...
class WXDLLIMPEXP_BASE wxLogStderr : public wxLog, protected wxMessageOutputStderr
{
public:
...
class WXDLLIMPEXP_BASE wxLogStream : public wxLog, private wxMessageOutputWithConv
{
public:
    // redirect log output to an ostream
  wxLogStream(std::ostream* ostr = (std::ostream*) NULL, const wxMBConv& conv = wxConvWhateverWorks);
protected:
    // implement sink function
  void DoLogText(const wxString& msg) override;
...
public:
  wxLogNull()
    : m_flagOld(wxLog::EnableLogging(false))
  {
  }
  ~wxLogNull()
  {
    (void) wxLog::EnableLogging(m_flagOld);
  }
private:
  bool m_flagOld;
};
...
public:
  ...
  wxLogger(wxLogLevel level, const char* filename, int line, const char* func, const char* component)
    : m_level(level)
  ...
  void LogV(const wxString& format, va_list argptr)
  {
    ...
    if (m_level == wxLOG_FatalError || wxLog::IsLevelEnabled(m_level, wxASCII_STR(m_info.component)))
    {
...
#    define wxLogVerbose	wxDO_IF(wxLOG_IS_ENABLED(Info) && wxLog::GetVerbose())                    \
    wxDO_LOG(Info)
...
#    define wxLogSysError	wxDO_LOG_IF_ENABLED_WITH_FUNC(Error, MaybeStore(wxLOG_KEY_SYS_ERROR_CODE, \
                                                    wxSysErrorCode()).Log)
...
#    define wxVLogSysError	wxMAKE_LOGGER(Error).MaybeStore(wxLOG_KEY_SYS_ERROR_CODE, \
                                    wxSysErrorCode()).LogV
#    if  wxUSE_GUI
...
#      define wxLogStatus	wxDO_LOG_IF_ENABLED_WITH_FUNC(Status, MaybeStore(wxLOG_KEY_FRAME).Log)
#      define wxVLogStatus	wxMAKE_LOGGER(Status).MaybeStore(wxLOG_KEY_FRAME).LogV
#    endif
#  else 
...
void WXDLLIMPEXP_BASE wxSafeShowMessage(const wxString& title, const wxString& text);
// ----------------------------------------------------------------------------
