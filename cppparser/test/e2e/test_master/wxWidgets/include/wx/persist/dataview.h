...
public:
  ...
  void Save() const override
  {
    ...
    for (unsigned int col = 0; col < control->GetColumnCount(); col++)
    {
      ...
      SaveValue(columnPrefix + wxASCII_STR(wxPERSIST_DVC_POS), control->GetColumnPosition(column));
    ...
    if (sortColumn)
    {
      ...
      SaveValue(wxASCII_STR(wxPERSIST_DVC_SORT_ASC), sortColumn->IsSortOrderAscending());
    }
  }
  bool Restore() override
  {
    ...
    for (unsigned int col = 0; col < control->GetColumnCount(); col++)
    {
      ...
      if (RestoreValue(columnPrefix + wxASCII_STR(wxPERSIST_DVC_WIDTH), &width))
      {
        ...
        // criteria.
    wxString sortColumn;
    if (control->GetModel() && RestoreValue(wxASCII_STR(wxPERSIST_DVC_SORT_KEY), &sortColumn) && !sortColumn.empty())
    {
      bool sortAsc = true;
