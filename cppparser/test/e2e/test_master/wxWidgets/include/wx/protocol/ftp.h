...
public:
  ...
  bool Connect(const wxString& host) override
  {
  ...
  bool SetBinary()
  {
    return SetTransferMode(BINARY);
  }
  bool SetAscii()
  {
    return SetTransferMode(ASCII);
  }
  ...
  const wxString& GetLastResult() const
  {
    return m_lastResult;
  }
...
protected:
  ...
  bool DoSimpleCommand(const wxChar* command, const wxString& arg = wxEmptyString);
