...
// wxDF_INVALID)
WXDLLIMPEXP_CORE extern const wxDataFormat& wxFormatInvalid;
// ----------------------------------------------------------------------------
...
public:
  ...
  virtual bool SetData(const wxDataFormat&, size_t, const void*)
  {
    return false;
  }
...
#    if  defined(__WXMSW__)
  const void* GetSizeFromBuffer(const void* buffer, size_t* size, const wxDataFormat& format) override;
...
public:
  ...
  wxBitmapDataObjectBase(const wxBitmap& bitmap = wxNullBitmap)
    : wxDataObjectSimple(wxDF_BITMAP)
    , m_bitmap(bitmap)
  {
  }
