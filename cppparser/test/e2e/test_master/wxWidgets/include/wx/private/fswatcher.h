...
public:
  ...
  virtual bool Add(const wxFSWatchInfo& winfo)
  {
    ...
    {
      wxLogTrace(wxTRACE_FSWATCHER, "Path '%s' is already watched", winfo.GetPath());
  ...
  virtual bool Remove(const wxFSWatchInfo& winfo)
  {
    ...
    if (it == m_watches.end())
    {
      wxLogTrace(wxTRACE_FSWATCHER, "Path '%s' is not watched", winfo.GetPath());
            // This can happen if a dir is watched, then a parent tree added
      return true;
    }
  ...
  {
    bool ret = true;
    for (wxFSWatchEntries::iterator it = m_watches.begin(); it != m_watches.end(); ++it)
    {
