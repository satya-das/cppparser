...
struct wxIDirectFBFont : public wxDfbWrapper<IDirectFBFont>
{
  ...
  bool GetStringExtents(const char* text, int bytes, DFBRectangle* logicalRect, DFBRectangle* inkRect)
  {
    return Check(m_ptr->GetStringExtents(m_ptr, text, bytes, logicalRect, inkRect));
  }
...
struct wxIDirectFBSurface : public wxDfbWrapper<IDirectFBSurface>
{
  ...
  {
    return Check(m_ptr->DrawString(m_ptr, text, bytes, x, y, (DFBSurfaceTextFlags) flags));
  }
    /**
  ...
  {
    return Check(m_ptr->SetBlittingFlags(m_ptr, (DFBSurfaceBlittingFlags) flags));
  }
  bool Blit(const wxIDirectFBSurfacePtr& source, const DFBRectangle* source_rect, int x, int y)
  {
    return Blit(source->GetRaw(), source_rect, x, y);
  }
  bool Blit(IDirectFBSurface* source, const DFBRectangle* source_rect, int x, int y)
  {
    return Check(m_ptr->Blit(m_ptr, source, source_rect, x, y));
  }
  bool StretchBlit(const wxIDirectFBSurfacePtr& source, const DFBRectangle* source_rect, const DFBRectangle* dest_rect)
  {
    return Check(m_ptr->StretchBlit(m_ptr, source->GetRaw(), source_rect, dest_rect));
  }
  ...
  enum CreateCompatibleFlags {
     ...
     */
  wxIDirectFBSurfacePtr CreateCompatible(const wxSize& size = wxDefaultSize, int flags = 0);
...
struct wxIDirectFB : public wxDfbWrapper<IDirectFB>
{
  ...
  wxIDirectFBFontPtr CreateFont(const char* filename, const DFBFontDescription* desc)
  {
    IDirectFBFont* f;
  ...
  wxIDirectFBDisplayLayerPtr GetDisplayLayer(DFBDisplayLayerID id = DLID_PRIMARY)
  {
