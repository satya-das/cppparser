...
#  endif
  ...
  {
    return 0x10000 <= value && value < 0x110000;
  }
  ...
  bool IsBMP() const
  {
    return IsBMP(m_value);
  }
    // Returns true if the character is a supplementary character:
  bool IsSupplementary() const
  {
    return IsSupplementary(m_value);
  }
    // Returns the high surrogate code unit for the supplementary character
  wxUint16 HighSurrogate() const
  {
    return HighSurrogate(m_value);
  }
    // Returns the low surrogate code unit for the supplementary character
  wxUint16 LowSurrogate() const
  {
    return LowSurrogate(m_value);
  }
  ...
  operator char() const
  {
    return To8bit(m_value);
  }
  operator unsigned char() const
  {
    return (unsigned char) To8bit(m_value);
  }
...
#  undef wxDEFINE_UNCHAR_CMP_WITH_INT
  ...
  {
    return m_value - From8bit(c);
  }
  int operator-(unsigned char c) const
  {
    return m_value - From8bit((char) c);
  }
  int operator-(wchar_t c) const
  {
    return m_value - (value_type) c;
  }
private:
...
#  if  wxUSE_UNICODE_UTF8
  wxUniCharRef(wxString& str, iterator pos)
    : m_str(str)
    , m_pos(pos)
  {
  }
#  else 
  wxUniCharRef(iterator pos)
    : m_pos(pos)
  {
  }
#  endif
...
#  endif
  wxUniChar::value_type GetValue() const
  {
    return UniChar().GetValue();
  }
#  if  wxUSE_UNICODE_UTF8
  wxUniChar::Utf8CharBuffer AsUTF8() const
  {
    return UniChar().AsUTF8();
  }
#  endif
  bool IsAscii() const
  {
    return UniChar().IsAscii();
  }
  bool GetAsChar(char* c) const
  {
    return UniChar().GetAsChar(c);
  }
  bool IsBMP() const
  {
    return UniChar().IsBMP();
  }
  bool IsSupplementary() const
  {
    return UniChar().IsSupplementary();
  }
  wxUint16 HighSurrogate() const
  {
    return UniChar().HighSurrogate();
  }
  wxUint16 LowSurrogate() const
  {
    return UniChar().LowSurrogate();
  }
    // Assignment operators:
#  if  wxUSE_UNICODE_UTF8
...
#  ifdef wxHAS_MEMBER_DEFAULT
  wxUniCharRef(const wxUniCharRef&);
#  endif
...
#  undef wxDEFINE_UNICHARREF_CMP_WITH_INT
  ...
  int operator-(const wxUniCharRef& c) const
  {
    return UniChar() - c.UniChar();
  }
  int operator-(const wxUniChar& c) const
  {
    return UniChar() - c;
...
inline void swap(wxUniCharRef&& lhs, wxUniCharRef&& rhs)
{
  wxUniChar tmp = lhs;
  lhs = rhs;
  rhs = tmp;
...
inline int operator-(char c1, const wxUniCharRef& c2)
{
  return -(c2 - c1);
}
inline int operator-(const wxUniChar& c1, const wxUniCharRef& c2)
{
  return -(c2 - c1);
}
inline int operator-(wchar_t c1, const wxUniCharRef& c2)
{
  return -(c2 - c1);
}
#endif
