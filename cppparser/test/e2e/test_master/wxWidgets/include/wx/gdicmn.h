...
public:
  ...
  wxSize& operator/=(unsigned int i)
  {
    x /= i;
    y /= i;
    return *this;
  ...
  wxSize& operator/=(unsigned long i)
  {
    x /= i;
    y /= i;
    return *this;
  ...
  {
    return x != wxDefaultCoord && y != wxDefaultCoord;
  }
...
public:
  ...
  wxRealPoint(double xx, double yy)
    : x(xx)
    , y(yy)
  {
  }
  ...
  wxRealPoint& operator+=(const wxRealPoint& p)
  {
    x += p.x;
    y += p.y;
    return *this;
  ...
  wxRealPoint& operator+=(const wxSize& s)
  {
    x += s.GetWidth();
    y += s.GetHeight();
    return *this;
...
public:
  ...
  wxPoint& operator+=(const wxSize& s)
  {
    x += s.GetWidth();
    y += s.GetHeight();
    return *this;
...
public:
  ...
  {
    return wxPoint(GetRight(), GetBottom());
  }
  ...
  {
    return wxPoint(GetRight(), GetTop());
  }
  ...
  {
    return wxPoint(GetLeft(), GetBottom());
  }
  ...
  wxRect& Deflate(const wxSize& d)
  {
    return Inflate(-d.x, -d.y);
  }
  wxRect& Deflate(wxCoord d)
  {
    return Inflate(-d);
  }
  ...
  {
    return wxRect(dir & wxHORIZONTAL ? r.x + (r.width - width) / 2 : x, dir & wxVERTICAL ? r.y + (r.height - height) / 2 : y, width, height);
  }
...
// order
inline void wxSize::IncBy(const wxPoint& pt)
{
  IncBy(pt.x, pt.y);
}
inline void wxSize::DecBy(const wxPoint& pt)
{
  DecBy(pt.x, pt.y);
}
// ---------------------------------------------------------------------------
...
public:
  ...
  wxResourceCache(unsigned int keyType)
    : wxList(keyType)
  {
  }
...
// 'Null' objects
WXDLLIMPEXP_CORE extern wxBitmap wxNullBitmap;
WXDLLIMPEXP_CORE extern wxIcon wxNullIcon;
WXDLLIMPEXP_CORE extern wxCursor wxNullCursor;
WXDLLIMPEXP_CORE extern wxPen wxNullPen;
WXDLLIMPEXP_CORE extern wxBrush wxNullBrush;
WXDLLIMPEXP_CORE extern wxPalette wxNullPalette;
WXDLLIMPEXP_CORE extern wxFont wxNullFont;
WXDLLIMPEXP_CORE extern wxColour wxNullColour;
WXDLLIMPEXP_CORE extern wxIconBundle wxNullIconBundle;
WXDLLIMPEXP_CORE extern wxColourDatabase* wxTheColourDatabase;
WXDLLIMPEXP_CORE extern const char wxPanelNameStr[];
