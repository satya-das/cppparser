...
#	include "geplin3d.h"
#	pragma  pack (push, 8)
class GE_DLLEXPIMPORT AcGeAugPolyline3d : public AcGePolyline3d
{
public:
	AcGeAugPolyline3d();
	AcGeAugPolyline3d(const AcGeAugPolyline3d& apline);
	AcGeAugPolyline3d(const AcGeKnotVector& knots, const AcGePoint3dArray& cntrlPnts, const AcGeVector3dArray& vecBundle);
	AcGeAugPolyline3d(const AcGePoint3dArray& cntrlPnts, const AcGeVector3dArray& vecBundle);
    // Approximation constructor
    //
	AcGeAugPolyline3d(const AcGeCurve3d& curve, double fromParam, double toParam, double apprEps);
    // Assignment operator.
    //
