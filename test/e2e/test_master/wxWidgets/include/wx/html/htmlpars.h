...
public:
  ...
  void DoParsing(const wxString::const_iterator& begin_pos, const wxString::const_iterator& end_pos);
  void DoParsing();
  ...
  wxHtmlEntitiesParser* GetEntitiesParser() const
  {
    return m_entitiesParser;
  }
  ...
  static bool SkipCommentTag(wxString::const_iterator& p, wxString::const_iterator end);
protected:
    // DOM structure
  void CreateDOMTree();
  void DestroyDOMTree();
  void CreateDOMSubTree(wxHtmlTag* cur, const wxString::const_iterator& begin_pos, const wxString::const_iterator& end_pos, wxHtmlTagsCache* cache);
