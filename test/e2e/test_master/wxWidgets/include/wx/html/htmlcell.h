...
public:
 ...
 return m_toPos;
  ...
  {
 m_toCharacterPos = m_fromCharacterPos = -1;
  }
  bool AreFromToCharacterPosSet() const
  {
 return m_toCharacterPos != -1 && m_fromCharacterPos != -1;
  }
  void SetFromCharacterPos(wxCoord pos)
  {
 m_fromCharacterPos = pos;
  }
  void SetToCharacterPos(wxCoord pos)
  {
 m_toCharacterPos = pos;
  }
  wxCoord GetFromCharacterPos() const
  {
 return m_fromCharacterPos;
  }
  wxCoord GetToCharacterPos() const
  {
 return m_toCharacterPos;
  }
  void SetExtentBeforeSelection(unsigned ext)
  {
 m_extBeforeSel = ext;
  }
  void SetExtentBeforeSelectionEnd(unsigned ext)
  {
 m_extBeforeSelEnd = ext;
  }
  unsigned GetExtentBeforeSelection() const
  {
 return m_extBeforeSel;
  }
  unsigned GetExtentBeforeSelectionEnd() const
  {
 return m_extBeforeSelEnd;
  }
  bool IsEmpty() const
  {
...
public:
 ...
 return *m_style;
  ...
  void SetCurrentUnderlined(bool u)
  {
 m_prevUnderlined = u;
  }
  bool WasPreviousUnderlined() const
  {
 return m_prevUnderlined;
  }
...
return m_Width;
  ...
  virtual int GetMaxTotalWidth() const
  {
 return m_Width;
  }
  int GetHeight() const
...
return m_Descent;
 ...
 m_id = id;
  ...
  virtual wxHtmlLinkInfo* GetLink(int = 0, int = 0) const
  {
 return m_Link;
  }
  ...
  virtual wxCursor GetMouseCursorAt(wxHtmlWindowInterface* window, const wxPoint& relPos) const;
...
m_Next = cell;
  ...
  virtual void Layout(int w);
    // renders the cell
  virtual void Draw(wxDC&, int, int, int, int, wxHtmlRenderingInfo&)
  {
  }
  ...
  virtual void DrawInvisible(wxDC&, int, int, wxHtmlRenderingInfo&)
  {
  }
 ...
 return wxConstCast(this, wxHtmlCell);
  ...
  virtual wxString ConvertToText(wxHtmlSelection*) const
  {
...
public:
  ...
  void Draw(wxDC& dc, int x, int y, int view_y1, int view_y2, wxHtmlRenderingInfo& info) override;
...
protected:
 ...
 return m_Word.Mid(begin, end - begin);
  ...
  void Split(const wxDC& dc, const wxPoint& selFrom, const wxPoint& selTo, unsigned& pos1, unsigned& pos2, unsigned& ext1, unsigned& ext2) const;
  wxString m_Word;
...
public:
  ...
  void Draw(wxDC& dc, int x, int y, int view_y1, int view_y2, wxHtmlRenderingInfo& info) override;
  void DrawInvisible(wxDC& dc, int x, int y, wxHtmlRenderingInfo& info) override;
...
m_BkColour = clr;
  ...
  {
m_Border = border; m_BorderColour1 = clr1; m_BorderColour2 = clr2;
  }
...
public:
 ...
 return m_pos != NULL;
  ...
  const wxHtmlCell* operator->() const
  {
 return m_pos;
  }
  const wxHtmlCell* operator*() const
