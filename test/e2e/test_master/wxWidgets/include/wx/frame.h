...
WXDLLIMPEXP_CORE extern const char wxStatusLineNameStr[];
...
public:
  ...
  wxFrameBase();
  virtual ~wxFrameBase();
  wxFrame* New(wxWindow* parent, wxWindowID winid, const wxString& title, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = wxDEFAULT_FRAME_STYLE, const wxString& name = wxASCII_STR(wxFrameNameStr));
    // frame state
    // -----------
...
#  if  wxUSE_STATUSBAR
  ...
  virtual wxStatusBar* CreateStatusBar(int number = 1, long style = wxSTB_DEFAULT_STYLE, wxWindowID winid = 0, const wxString& name = wxASCII_STR(wxStatusLineNameStr));
