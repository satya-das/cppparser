...
#  if  wxUSE_IPV6
template <>
struct AddressFamily<sockaddr_in6>
{
  enum { value = AF_INET6 };
};
#  endif
...
template <>
struct AddressFamily<sockaddr_un>
{
  enum { value = AF_UNIX };
};
#  endif
...
private:
  ...
  T* Get() const
  {
    wxCHECK_MSG(static_cast<int>(m_family) == AddressFamily<T>::value, NULL, "socket address family mismatch");
    return reinterpret_cast<T*>(m_addr);
  }
  void InitUnspec()
  {
