...
public:
  ...
  bool GetGroupRange(int pos, int* start, int* end) const
  {
    ...
    for (Ranges::const_iterator it = m_ranges.begin(); it != m_ranges.end(); ++it)
    {
      const Range& r = *it;
  ...
  bool UpdateOnInsertRadio(int pos)
  {
    ...
    for (Ranges::iterator it = m_ranges.begin(); it != m_ranges.end(); ++it)
    {
      Range& r = *it;
      if (pos < r.start)
      {
      ...
      else 
      {
        if (pos <= r.end + 1)
        {
          wxASSERT_MSG(!inExistingGroup, wxS("Item already inserted inside another range"));
  ...
  bool UpdateOnInsertNonRadio(int pos)
  {
    ...
    for (Ranges::iterator it = m_ranges.begin(); it != m_ranges.end(); ++it)
    {
      ...
      else 
      {
        if (pos <= r.end)
        {
          wxASSERT_MSG(!wasSplit, wxS("Item already inserted inside another range"));
          ...
          newRange.start = pos + 1;
          newRange.end = r.end + 1;
          wasSplit = true;
