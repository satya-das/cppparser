...
{
  int pad_int;
  long int pad_longint;
#  ifdef wxLongLong_t
  wxLongLong_t pad_longlongint;
#  endif
  size_t pad_sizet;
  double pad_double;
  long double pad_longdouble;
  void* pad_pointer;
  char pad_char;
  wchar_t pad_wchar;
  void* pad_str;
  int* pad_nint;
  short int* pad_nshortint;
  long int* pad_nlongint;
};
...
template <>
struct wxPrintfStringHelper<char>
{
...
bool wxPrintfConvSpec<CharType>::Parse(const CharType* format)
{
  ...
  do
  {
#  define CHECK_PREC	if (in_prec && !prec_dot) \
        { \
            m_szFlags[flagofs++] = '.'; \
            prec_dot = true; \
        }
...
#  ifdef __WINDOWS__
      ...
      case wxT('I'):
        if (*(m_pArgEnd + 1) == wxT('6') && *(m_pArgEnd + 2) == wxT('4'))
        {
          m_pArgEnd++;
          m_pArgEnd++;
          ilen = 2;
...
#  endif
      ...
      case wxT('9'):
        int len = 0;
        while ((*m_pArgEnd >= CharType('0')) && (*m_pArgEnd <= CharType('9')))
        {
      ...
      case wxT('$'):
        ...
        {
          flagofs--;
        } while (m_szFlags[flagofs] >= '1' && m_szFlags[flagofs] <= '9');
...
#  ifndef SYSTEM_SPRINTF_IS_UNSAFE
  int maxlen = (m_szFlags + wxMAX_SVNPRINTF_FLAGBUFFER_LEN - pwidth) / sizeof(*m_szFlags);
#  endif
...
#  endif
    ...
    case wxPAT_POINTER:
      ...
      break;
    case wxPAT_CHAR:
      p->pad_char = (char) va_arg(argptr, int);
      break;
    case wxPAT_WCHAR:
      p->pad_wchar = (wchar_t) va_arg(argptr, int);
      break;
    case wxPAT_PCHAR:
    case wxPAT_PWCHAR:
...
#  endif
    ...
    case wxPAT_PWCHAR:
      ...
      typename wxPrintfStringHelper<CharType>::ConvertedType strbuf(wxPrintfStringHelper<CharType>::Convert(s));
...
struct wxPrintfConvSpecParser
{
  ...
  {
    nspecs = nargs = 0;
    posarg_present = nonposarg_present = false;
    ...
    for (const CharType* toparse = fmt; *toparse != wxT('\0'); toparse++)
    {
      ...
      if (const char* f = strchr(spec->m_szFlags, '*'))
      {
        ...
        for (unsigned n = 0; n < numAsterisks; n++)
        {
          ...
          if (posarg_present)
          {
            wxFAIL_MSG(wxString::Format(wxASCII_STR("Format string \"%s\" uses both positional "
                                "parameters and '*' but this is not currently "
                                "supported by this implementation, sorry."), fmt));
          }
          specs[nspecs] = *spec;
      ...
      if (spec->m_pos > 0)
      {
        ...
        else 
        {
          if (pspec[spec->m_pos])
          {
            ...
            wxASSERT_MSG(pspec[spec->m_pos]->m_type == spec->m_type, "Positional parameter specified multiple times "
    ...
    if (nspecs == wxMAX_SVNPRINTF_ARGUMENTS)
    {
      wxFAIL_MSG(wxString::Format(wxASCII_STR("wxVsnprintf() currently supports only %d arguments, "
                    "but format string \"%s\" defines more of them.\n"
                    "You need to change wxMAX_SVNPRINTF_ARGUMENTS and "
                    "recompile if more are really needed."), fmt, wxMAX_SVNPRINTF_ARGUMENTS));
    }
  }
