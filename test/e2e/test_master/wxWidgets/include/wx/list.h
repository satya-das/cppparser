...
// -----------------------------------------------------------------------------
WXDLLIMPEXP_BASE extern wxListKey wxDefaultListKey;
...
public:
    // ctor
  wxNodeBase(wxListBase* list = NULL, wxNodeBase* previous = NULL, wxNodeBase* next = NULL, void* data = NULL, const wxListKey& key = wxDefaultListKey);
  virtual ~wxNodeBase();
...
protected:
  ...
  virtual wxNodeBase* CreateNode(wxNodeBase* prev, wxNodeBase* next, void* data, const wxListKey& key = wxDefaultListKey) = 0;
  ...
  {
    return pos == GetCount() ? Append(object) : Insert(Item(pos), object);
  }
...
private:
  ...
  size_t m_count;
  bool m_destroy;
  wxNodeBase* m_nodeFirst, *m_nodeLast;
  wxKeyType m_keyType;
};
// -----------------------------------------------------------------------------
...
#  define WX_LIST_PTROP	pointer_type operator->() const                                 \
                { return (pointer_type)m_node->GetDataPtr(); }
...
operator wxList&()
{
  return *static_cast<wxList*>(this);
}
operator const wxList&() const
{
  return *static_cast<const wxList*>(this);
}
// -----------------------------------------------------------------------------
...
#    endif
        ...
        // inefficient!)
  wxStringList(const wxStringList& other)
    : wxStringListBase()
  {
    DeleteContents(true);
    DoCopy(other);
