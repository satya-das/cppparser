...
{
public:
  wxZipEntry(const wxString& name = wxEmptyString, const wxDateTime& dt = wxDateTime::Now(), wxFileOffset size = wxInvalidOffset);
  virtual ~wxZipEntry();
  ...
  inline void SetName(const wxString& name, wxPathFormat format = wxPATH_NATIVE) override;
...
protected:
  ...
  int GetVersionNeeded() const
  { return m_VersionNeeded; }
  wxFileOffset GetKey() const
  { return m_Key; }
  int GetVersionMadeBy() const
  { return m_VersionMadeBy; }
  int GetDiskStart() const
  { return m_DiskStart; }
  int GetInternalAttributes() const
  ...
  { m_VersionMadeBy = (wxUint8)version; }
  void SetCrc(wxUint32 crc)
  { m_Crc = crc; }
  void SetCompressedSize(wxFileOffset size)
  { m_CompressedSize = size; }
  void SetKey(wxFileOffset offset)
  { m_Key = offset; }
  void SetDiskStart(int start)
  { m_DiskStart = (wxUint16)start; }
  void SetInternalAttributes(int attr)
  { m_InternalAttributes = (wxUint16)attr; }
  virtual wxZipEntry* ZipClone() const
  { return new wxZipEntry(*this); }
  void Notify();
private:
  ...
  size_t WriteDescriptor(wxOutputStream& stream, wxUint32 crc, wxFileOffset compressedSize, wxFileOffset size);
  ...
  wxUint8 m_SystemMadeBy;
  wxUint8 m_VersionMadeBy;
  wxUint16 m_VersionNeeded;
  wxUint16 m_Flags;
  wxUint16 m_Method;
  ...
  wxString m_Comment;
  wxUint16 m_DiskStart;
  wxUint16 m_InternalAttributes;
  wxUint32 m_ExternalAttributes;
  wxUint16 m_z64infoOffset;
...
public:
  ...
  wxZipOutputStream(wxOutputStream* stream, int level = -1, wxMBConv& conv = wxConvUTF8);
  WXZIPFIX virtual ~wxZipOutputStream();
  bool PutNextEntry(wxZipEntry* entry)
  { return DoCreate(entry); }
  bool WXZIPFIX PutNextEntry(const wxString& name, const wxDateTime& dt = wxDateTime::Now(), wxFileOffset size = wxInvalidOffset) override;
  bool WXZIPFIX PutNextDirEntry(const wxString& name, const wxDateTime& dt = wxDateTime::Now()) override;
...
protected:
  ...
  virtual wxOutputStream* WXZIPFIX OpenCompressor(wxOutputStream& stream, wxZipEntry& entry, const Buffer bufs[]);
...
public:
  ...
  wxZipInputStream(wxInputStream* stream, wxMBConv& conv = wxConvLocal);
  WXZIPFIX virtual ~wxZipInputStream();
...
private:
  ...
  bool AfterHeader() const
  { return m_headerSize > 0 && !m_decomp; }
  bool IsOpened() const
  { return m_decomp != NULL; }
  ...
  wxString m_Comment;
  friend bool wxZipOutputStream::CopyEntry(wxZipEntry* entry, wxZipInputStream& inputStream);
  friend bool wxZipOutputStream::CopyArchiveMetaData(wxZipInputStream& inputStream);
...
inline void wxZipEntry::SetName(const wxString& name, wxPathFormat format)
{
    bool isDir;
