...
WXDLLIMPEXP_CORE extern const char wxVListBoxNameStr[];
// ----------------------------------------------------------------------------
// wxVListBox
// ----------------------------------------------------------------------------
...
public:
  ...
  bool Create(wxWindow* parent, wxWindowID id = wxID_ANY, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = 0, const wxString& name = wxASCII_STR(wxVListBoxNameStr));
  ...
  {
    wxASSERT_MSG(!HasMultipleSelection(), wxT("GetSelection() can't be used with wxLB_MULTIPLE"));
    return m_current;
  }
...
protected:
  ...
  wxCoord OnGetRowHeight(size_t line) const override;
    // event handlers
  ...
  void DoHandleItemClick(int item, int flags);
    ...
    // OnDrawBackground())
  bool DoDrawSolidBackground(const wxColour& col, wxDC& dc, const wxRect& rect, size_t n) const;
private:
