...
// constants
// ----------------------------------------------------------------------------
WXDLLIMPEXP_CORE extern const char wxGridNameStr[];
...
public:
  ...
  virtual wxSize GetBestSize(wxGrid& grid, wxGridCellAttr& attr, wxDC& dc, int row, int col) = 0;
    ...
    // and vice versa
  virtual int GetBestHeight(wxGrid& grid, wxGridCellAttr& attr, wxDC& dc, int row, int col, int)
  {
  ...
  virtual int GetBestWidth(wxGrid& grid, wxGridCellAttr& attr, wxDC& dc, int row, int col, int)
  {
    ...
    // default.
  virtual wxSize GetMaxBestSize(wxGrid&, wxGridCellAttr&, wxDC&)
  {
...
private:
  explicit wxGridActivationResult(Action action, const wxString& newval = wxString())
    : m_action(action)
    , m_newval(newval)
  {
  }
...
public:
  ...
  wxGridCellAttr* GetCellAttr() const
  {
    return m_attr;
  }
  ...
  {
    return wxDynamicCast(m_control, wxControl);
  }
  ...
  virtual wxGridActivationResult TryActivate(int, int, wxGrid*, const wxGridActivationSource&)
  {
    return wxGridActivationResult::DoEdit();
  }
  virtual void DoActivate(int, int, wxGrid*)
  {
...
protected:
  ...
  void DoPositionEditor(const wxSize& size, const wxRect& rectCell, int hAlign = wxALIGN_LEFT, int vAlign = wxALIGN_CENTRE_VERTICAL);
  ...
  wxColour m_colFgOld, m_colBgOld;
  wxFont m_fontOld;
    // suppress the stupid gcc warning about the class having private dtor and
    // no friends
...
public:
  ...
  wxGridActivationResult TryActivate(int row, int col, wxGrid* grid, const wxGridActivationSource& actSource) = 0;
  void DoActivate(int row, int col, wxGrid* grid) = 0;
  ...
  bool EndEdit(int, int, const wxGrid*, const wxString&, wxString*) override
  {
    wxFAIL;
    return false;
  }
...
class WXDLLIMPEXP_CORE wxGridRowHeaderRenderer : public wxGridHeaderLabelsRenderer
{
};
class WXDLLIMPEXP_CORE wxGridColumnHeaderRenderer : public wxGridHeaderLabelsRenderer
{
};
class WXDLLIMPEXP_CORE wxGridCornerHeaderRenderer : public wxGridHeaderLabelsRenderer
{
};
// Also define the default renderers which are used by wxGridCellAttrProvider
// by default
class WXDLLIMPEXP_CORE wxGridRowHeaderRendererDefault : public wxGridRowHeaderRenderer
{
public:
  void DrawBorder(const wxGrid& grid, wxDC& dc, wxRect& rect) const override;
};
// Column header cells renderers
class WXDLLIMPEXP_CORE wxGridColumnHeaderRendererDefault : public wxGridColumnHeaderRenderer
{
public:
  void DrawBorder(const wxGrid& grid, wxDC& dc, wxRect& rect) const override;
};
// Header corner renderer
class WXDLLIMPEXP_CORE wxGridCornerHeaderRendererDefault : public wxGridCornerHeaderRenderer
{
public:
  void DrawBorder(const wxGrid& grid, wxDC& dc, wxRect& rect) const override;
};
...
public:
  ...
  {
    return wxGridFitMode(Mode_Clip);
  }
  static wxGridFitMode Overflow()
  {
    return wxGridFitMode(Mode_Overflow);
  }
  ...
  {
    return m_mode != Mode_Unset;
  }
  bool IsClip() const
  {
    return m_mode == Mode_Clip;
  }
  bool IsOverflow() const
  {
    return m_mode == Mode_Overflow;
  }
...
private:
  ...
  explicit wxGridFitMode(Mode mode)
    : m_mode(mode)
  {
  }
  Mode m_mode;
...
private:
  ...
  wxColour m_colText, m_colBack;
  wxFont m_font;
  int m_hAlign, m_vAlign;
...
public:
  ...
  {
    return m_topRow <= other.m_bottomRow && m_bottomRow >= other.m_topRow && m_leftCol <= other.m_rightCol && m_rightCol >= other.m_leftCol;
  }
    // Return whether this block contains the given cell.
  bool Contains(const wxGridCellCoords& cell) const
  {
    return m_topRow <= cell.GetRow() && cell.GetRow() <= m_bottomRow && m_leftCol <= cell.GetCol() && cell.GetCol() <= m_rightCol;
  }
    // Return whether this blocks fully contains another one.
  bool Contains(const wxGridBlockCoords& other) const
  {
    return m_topRow <= other.m_topRow && other.m_bottomRow <= m_bottomRow && m_leftCol <= other.m_leftCol && other.m_rightCol <= m_rightCol;
  }
  ...
  {
    return m_topRow == other.m_topRow && m_leftCol == other.m_leftCol && m_bottomRow == other.m_bottomRow && m_rightCol == other.m_rightCol;
  }
  bool operator!=(const wxGridBlockCoords& other) const
  {
    return !(*this == other);
  ...
  bool operator!() const
  {
    return m_topRow == -1 && m_leftCol == -1 && m_bottomRow == -1 && m_rightCol == -1;
  }
private:
  int m_topRow;
  int m_leftCol;
...
#    endif
  ...
  private:
    explicit iterator(iterator_impl it)
      : m_it(it)
    {
    }
    iterator_impl m_it;
...
// For comparisons...
//
WXDLLIMPEXP_CORE extern wxGridCellCoords wxGridNoCellCoords;
WXDLLIMPEXP_CORE extern wxGridBlockCoords wxGridNoBlockCoords;
WXDLLIMPEXP_CORE extern wxRect wxGridNoCellRect;
...
class WXDLLIMPEXP_CORE wxGridTableBase : public wxObject, public wxClientDataContainer
{
public:
  wxGridTableBase();
  ...
  virtual void SetRowLabelValue(int, const wxString&)
  {
  }
  ...
  wxGridCellAttrPtr GetAttrPtr(int row, int col, wxGridCellAttr::wxAttrKind kind)
  {
    ...
    // this table.
  virtual bool CanMeasureColUsingSameAttr(int) const
  {
    return false;
  }
...
public:
  ...
  wxGrid(wxWindow* parent, wxWindowID id, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = wxWANTS_CHARS, const wxString& name = wxASCII_STR(wxGridNameStr))
  {
    Init();
    Create(parent, id, pos, size, style, name);
  }
  bool Create(wxWindow* parent, wxWindowID id, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = wxWANTS_CHARS, const wxString& name = wxASCII_STR(wxGridNameStr));
  virtual ~wxGrid();
  ...
  {
    return m_numFrozenRows;
  }
  int GetNumberFrozenCols() const
  {
    return m_numFrozenCols;
  }
    // ------ display update functions
    //
  wxArrayInt CalcRowLabelsExposed(const wxRegion& reg, wxGridWindow* gridWindow = NULL) const;
  wxArrayInt CalcColLabelsExposed(const wxRegion& reg, wxGridWindow* gridWindow = NULL) const;
  wxGridCellCoordsArray CalcCellsExposed(const wxRegion& reg, wxGridWindow* gridWindow = NULL) const;
  void PrepareDCFor(wxDC& dc, wxGridWindow* gridWindow);
  void ClearGrid();
  bool InsertRows(int pos = 0, int numRows = 1, bool updateLabels = true)
  {
    return DoModifyLines(&wxGridTableBase::InsertRows, pos, numRows, updateLabels);
  }
  bool InsertCols(int pos = 0, int numCols = 1, bool updateLabels = true)
  {
    return DoModifyLines(&wxGridTableBase::InsertCols, pos, numCols, updateLabels);
  }
  ...
  {
    return DoModifyLines(&wxGridTableBase::DeleteRows, pos, numRows, updateLabels);
  }
  bool DeleteCols(int pos = 0, int numCols = 1, bool updateLabels = true)
  {
    return DoModifyLines(&wxGridTableBase::DeleteCols, pos, numCols, updateLabels);
  }
  ...
  virtual void DrawCornerLabel(wxDC& dc);
    ...
    //
  void DrawTextRectangle(wxDC& dc, const wxString&, const wxRect&, int horizontalAlignment = wxALIGN_LEFT, int verticalAlignment = wxALIGN_TOP, int textOrientation = wxHORIZONTAL) const;
  void DrawTextRectangle(wxDC& dc, const wxArrayString& lines, const wxRect&, int horizontalAlignment = wxALIGN_LEFT, int verticalAlignment = wxALIGN_TOP, int textOrientation = wxHORIZONTAL) const;
  void DrawTextRectangle(wxDC& dc, const wxString& text, const wxRect& rect, const wxGridCellAttr& attr, int defaultHAlign = wxALIGN_INVALID, int defaultVAlign = wxALIGN_INVALID);
  ...
  void GetTextBoxSize(const wxDC& dc, const wxArrayString& lines, long* width, long* height) const;
  ...
  void XYToCell(int x, int y, wxGridCellCoords& coords, wxGridWindow* gridWindow = NULL) const
  {
  ...
  void SetGridCursor(const wxGridCellCoords& c)
  {
    SetCurrentCell(c);
  }
  ...
  {
    return FromDIP(WXGRID_DEFAULT_COL_LABEL_HEIGHT);
  }
  ...
  {
    return m_gridFrozenBorderColour;
  }
  int GetGridFrozenBorderPenWidth() const
  {
    return m_gridFrozenBorderPenWidth;
  }
  ...
  {
    DoDisableLineResize(row, m_setFixedRows);
  }
  void DisableColResize(int col)
  {
    DoDisableLineResize(col, m_setFixedCols);
  }
  ...
  {
    EnableHidingColumns(false);
  }
  bool CanHideColumns() const
  {
    return m_canHideColumns;
  }
  ...
  {
    SelectBlock(topLeft.GetRow(), topLeft.GetCol(), bottomRight.GetRow(), bottomRight.GetCol(), addToSelected);
  }
  void SelectAll();
  ...
  {
    m_selectionBackground = c;
  }
  void SetSelectionForeground(const wxColour& c)
  {
    m_selectionForeground = c;
  }
  ...
  {
    return (wxWindow*) m_frozenCornerGridWin;
  }
  wxWindow* GetFrozenRowGridWindow() const
  {
    return (wxWindow*) m_frozenRowGridWin;
  }
  wxWindow* GetFrozenColGridWindow() const
  {
    return (wxWindow*) m_frozenColGridWin;
  }
  ...
  {
    m_xScrollPixelsPerLine = x;
  }
  void SetScrollLineY(int y)
  {
    m_yScrollPixelsPerLine = y;
  }
...
protected:
  ...
  bool m_useNativeHeader, m_nativeColumnLabels;
...
protected:
  ...
  int m_extraWidth, m_extraHeight;
  ...
  bool m_gridLinesClipHorz, m_gridLinesClipVert;
  ...
  wxCoord CalcColOrRowLabelAreaMinSize(wxGridDirection direction);
    ...
    // hash table
  wxLongToLongHashMap m_colMinWidths, m_rowMinHeights;
  ...
  struct CachedAttr
{
  ...
  enum CursorMode {
    ...
    // directly!
  void ChangeCursorMode(CursorMode mode, wxWindow* win = NULL, bool captureMouse = true);
  ...
  EventResult SendEvent(wxEventType evtType, int row, int col, const wxMouseEvent& e);
  EventResult SendEvent(wxEventType evtType, const wxGridCellCoords& coords, const wxMouseEvent& e)
  {
    return SendEvent(evtType, coords.GetRow(), coords.GetCol(), e);
  }
  EventResult SendEvent(wxEventType evtType, int row, int col, const wxString& s = wxString());
  EventResult SendEvent(wxEventType evtType, const wxGridCellCoords& coords, const wxString& s = wxString())
  {
  ...
  bool SendGridSizeEvent(wxEventType type, int row, int col, const wxMouseEvent& mouseEv);
  ...
  bool ShouldScrollToChildOnFocus(wxWindow*) override
  {
    return false;
  }
...
private:
  ...
  void DrawRangeGridLines(wxDC& dc, const wxRegion& reg, const wxGridCellCoords& topLeft, const wxGridCellCoords& bottomRight);
  ...
  void DoDrawGridLines(wxDC& dc, int top, int left, int bottom, int right, int topRow, int leftCol, int bottomRight, int rightCol);
  ...
  bool DoGridCellDrag(wxMouseEvent& event, const wxGridCellCoords& coords, bool isFirstDrag);
    ...
    // handler
  bool DoGridDragEvent(wxMouseEvent& event, const wxGridCellCoords& coords, bool isFirstDrag, wxGridWindow* gridWindow);
    // Update the width/height of the column/row being drag-resized.
  void DoGridDragResize(const wxPoint& position, const wxGridOperations& oper, wxGridWindow* gridWindow);
    // process different clicks on grid cells
  void DoGridCellLeftDown(wxMouseEvent& event, const wxGridCellCoords& coords, const wxPoint& pos);
  void DoGridCellLeftDClick(wxMouseEvent& event, const wxGridCellCoords& coords, const wxPoint& pos);
  void DoGridCellLeftUp(wxMouseEvent& event, const wxGridCellCoords& coords, wxGridWindow* gridWindow);
    // process movement (but not dragging) event in the grid cell area
  void DoGridMouseMoveEvent(wxMouseEvent& event, const wxGridCellCoords& coords, const wxPoint& pos, wxGridWindow* gridWindow);
  ...
  void ProcessRowLabelMouseEvent(wxMouseEvent& event, wxGridRowLabelWindow* rowLabelWin);
  void ProcessColLabelMouseEvent(wxMouseEvent& event, wxGridColLabelWindow* colLabelWin);
  ...
  int PosToLinePos(int pos, bool clipToMinMax, const wxGridOperations& oper, wxGridWindow* gridWindow) const;
  int PosToLine(int pos, bool clipToMinMax, const wxGridOperations& oper, wxGridWindow* gridWindow) const;
  int PosToEdgeOfLine(int pos, const wxGridOperations& oper) const;
  void DoMoveCursorFromKeyboard(const wxKeyboardState& kbdState, const wxGridDirectionOperations& diroper);
  bool DoMoveCursor(const wxKeyboardState& kbdState, const wxGridDirectionOperations& diroper);
  bool DoMoveCursorByPage(const wxKeyboardState& kbdState, const wxGridDirectionOperations& diroper);
  bool AdvanceByPage(wxGridCellCoords& coords, const wxGridDirectionOperations& diroper);
  bool DoMoveCursorByBlock(const wxKeyboardState& kbdState, const wxGridDirectionOperations& diroper);
  void AdvanceToNextNonEmpty(wxGridCellCoords& coords, const wxGridDirectionOperations& diroper);
  bool AdvanceByBlock(wxGridCellCoords& coords, const wxGridDirectionOperations& diroper);
    // common part of {Insert,Delete}{Rows,Cols}
  bool DoModifyLines(bool (*funcModify) (size_t, size_t), int pos, int num, bool updateLabels);
    // Append{Rows,Cols} is a bit different because of one less parameter
  bool DoAppendLines(bool (*funcAppend) (size_t), int num, bool updateLabels);
    // common part of Set{Col,Row}Sizes
  void DoSetSizes(const wxGridSizesInfo& sizeInfo, const wxGridOperations& oper);
  ...
  void GetRenderSizes(const wxGridCellCoords& topLeft, const wxGridCellCoords& bottomRight, wxPoint& pointOffSet, wxSize& sizeGrid, wxGridCellCoordsArray& renderCells, wxArrayInt& arrayCols, wxArrayInt& arrayRows);
    // Helper of Render(): set the scale to draw the cells at the right size.
  void SetRenderScale(wxDC& dc, const wxPoint& pos, const wxSize& size, const wxSize& sizeGrid);
  ...
  void DoRenderBox(wxDC& dc, const int& style, const wxPoint& pointOffSet, const wxSize& sizeCellArea, const wxGridCellCoords& topLeft, const wxGridCellCoords& bottomRight);
  ...
  {
    return GetCellAttrPtr(m_currentCellCoords)->GetEditorPtr(this, m_currentCellCoords.GetRow(), m_currentCellCoords.GetCol());
  }
  ...
  wxGridFixedIndicesSet* m_setFixedRows, *m_setFixedCols;
...
// ----------------------------------------------------------------------------
class WXDLLIMPEXP_CORE wxGridEvent : public wxNotifyEvent, public wxKeyboardState
{
public:
  wxGridEvent()
    : wxNotifyEvent()
...
class WXDLLIMPEXP_CORE wxGridSizeEvent : public wxNotifyEvent, public wxKeyboardState
{
public:
  wxGridSizeEvent()
    : wxNotifyEvent()
  ...
  wxEvent* Clone() const override
  {
    return new wxGridSizeEvent(*this);
  }
protected:
  int m_rowOrCol;
  int m_x;
...
class WXDLLIMPEXP_CORE wxGridRangeSelectEvent : public wxNotifyEvent, public wxKeyboardState
{
public:
  ...
  wxEvent* Clone() const override
  {
    return new wxGridRangeSelectEvent(*this);
  }
protected:
  void Init(const wxGridCellCoords& topLeft, const wxGridCellCoords& bottomRight, bool selecting)
  {
    m_topLeft = topLeft;
...
public:
  ...
  wxGridEditorCreatedEvent(int id, wxEventType type, wxObject* obj, int row, int col, wxWindow* window);
  int GetRow()
  {
    return m_row;
  }
  ...
  wxControl* GetControl()
  {
    return wxDynamicCast(m_window, wxControl);
  }
  ...
  wxEvent* Clone() const override
  {
    return new wxGridEditorCreatedEvent(*this);
  }
private:
  int m_row;
  int m_col;
...
typedef void (*wxGridEventFunction) (wxGridEvent&);
typedef void (*wxGridSizeEventFunction) (wxGridSizeEvent&);
typedef void (*wxGridRangeSelectEventFunction) (wxGridRangeSelectEvent&);
typedef void (*wxGridEditorCreatedEventFunction) (wxGridEditorCreatedEvent&);
