...
public:
  ...
  {
    return m_grid->GetColMinimalWidth(m_col);
  }
...
public:
  wxGridHeaderCtrl(wxGrid* owner)
    : wxHeaderCtrl(owner, wxID_ANY, wxDefaultPosition, wxDefaultSize, (owner->CanHideColumns() ? wxHD_ALLOW_HIDE : 0) | (owner->CanDragColMove() ? wxHD_ALLOW_REORDER : 0))
  {
    m_inResizing = 0;
  }
...
protected:
  ...
  wxGrid* GetOwner() const
  {
    return static_cast<wxGrid*>(GetParent());
  }
private:
  ...
  bool UpdateColumnWidthToFit(unsigned int idx, int) override
  {
  ...
  void UpdateColumnVisibility(unsigned int idx, bool show) override
  {
    GetOwner()->SetColSize(idx, show ? wxGRID_AUTOSIZE : 0);
        ...
        // it
    GetOwner()->SendGridSizeEvent(wxEVT_GRID_COL_SIZE, -1, idx, GetDummyMouseEvent());
  }
  ...
  {
    GetOwner()->SendEvent(wxEVT_GRID_LABEL_LEFT_CLICK, -1, event.GetColumn(), GetDummyMouseEvent());
    GetOwner()->DoColHeaderClick(event.GetColumn());
  }
  void OnDoubleClick(wxHeaderCtrlEvent& event)
  {
    if (!GetOwner()->SendEvent(wxEVT_GRID_LABEL_LEFT_DCLICK, -1, event.GetColumn(), GetDummyMouseEvent()))
    {
      event.Skip();
    }
  }
  ...
  {
    if (!GetOwner()->SendEvent(wxEVT_GRID_LABEL_RIGHT_CLICK, -1, event.GetColumn(), GetDummyMouseEvent()))
    {
      event.Skip();
    }
  }
...
{
public:
  wxGridSubwindow(wxGrid* owner, int additionalStyle = 0, const wxString& name = wxASCII_STR(wxPanelNameStr))
    : wxWindow(owner, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE | additionalStyle, name)
  {
    m_owner = owner;
  }
...
public:
  ...
  wxGridWindow(wxGrid* parent, wxGridWindowType type)
    : wxGridSubwindow(parent, wxWANTS_CHARS | wxCLIP_CHILDREN, "GridWindow")
    , m_type(type)
  {
  ...
  wxGridWindowType GetType() const
  {
    return m_type;
  }
private:
...
public:
  ...
  wxGridRowOrColAttrData m_rowAttrs, m_colAttrs;
};
// ----------------------------------------------------------------------------
...
public:
  ...
  virtual void DrawParallelLine(wxDC& dc, int start, int end, int pos) const = 0;
  ...
  virtual int PosToLine(const wxGrid* grid, int pos, wxGridWindow* gridWindow, bool clip = false) const = 0;
...
public:
  ...
  int Select(const wxGridCellCoords& c) const override
  {
    return c.GetRow();
  }
  int Select(const wxPoint& pt) const override
  {
    return pt.x;
  }
  int Select(const wxSize& sz) const override
  {
    return sz.x;
  }
  int Select(const wxRect& r) const override
  {
    return r.x;
  }
  int& Select(wxRect& r) const override
  {
    return r.x;
  }
  ...
  int& SelectSize(wxRect& r) const override
  {
    return r.width;
  }
  ...
  int GetLineAt(const wxGrid*, int pos) const override
  {
    return pos;
  }
  int GetLinePos(const wxGrid*, int line) const override
  {
    return line;
  }
  int GetLineBefore(const wxGrid*, int line) const override
  {
    return line - 1;
  }
...
public:
  ...
  void Advance(wxGridCellCoords& coords) const override
  {
    ...
    for (;;)
    {
      wxCHECK_RET(pos < m_numLines - 1, "can't advance when already at boundary");
      int line = GetLineAt(++pos);
      if (IsLineVisible(line))
      {
        m_oper.Set(coords, line);
...
{
  wxGridDataTypeInfo(const wxString& typeName, wxGridCellRenderer* renderer, wxGridCellEditor* editor)
    : m_typeName(typeName)
    , m_renderer(renderer)
    , m_editor(editor)
  {
...
// fallback. If this still fails, returns false.
  bool TryGetValueAsDate(wxDateTime& result, const DateParseParams& params, const wxGrid& grid, int row, int col);
}
#  endif
#endif
