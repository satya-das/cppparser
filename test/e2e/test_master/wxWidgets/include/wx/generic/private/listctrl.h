  ...
  struct GeometryInfo
{
  ...
  {
    wxASSERT_MSG(m_rectAll.width <= w, wxT("width can only be increased"));
    m_rectAll.width = w;
  ...
  void DrawInReportMode(wxDC* dc, const wxRect& rect, const wxRect& rectHL, bool highlighted, bool current);
private:
  ...
  void ApplyAttributes(wxDC* dc, const wxRect& rectHL, bool highlighted, bool current);
  ...
  void DrawTextFormatted(wxDC* dc, const wxString& text, int col, int x, int yMid, int width);
};
...
public:
  ...
  bool Create(wxWindow* win, wxWindowID id, wxListMainWindow* owner, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = 0, const wxString& name = wxT("wxlistctrlcolumntitles"));
...
public:
  wxListMainWindow();
  wxListMainWindow(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size);
  ...
  {
    HighlightLine(line, !IsHighlighted(line));
    RefreshLine(line);
  }
...
protected:
  ...
  size_t m_lineLastClicked, m_lineBeforeLastClicked, m_lineSelectSingleOnUp;
...
private:
  ...
  size_t m_lineFrom, m_lineTo;
    // the brushes to use for item highlighting when we do/don't have focus
  wxBrush* m_highlightBrush, *m_highlightUnfocusedBrush;
