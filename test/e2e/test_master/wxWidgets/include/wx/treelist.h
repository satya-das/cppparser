...
WXDLLIMPEXP_CORE extern const char wxTreeListCtrlNameStr[];
...
WXDLLIMPEXP_CORE extern const wxTreeListItem wxTLI_FIRST;
WXDLLIMPEXP_CORE extern const wxTreeListItem wxTLI_LAST;
// ----------------------------------------------------------------------------
...
class WXDLLIMPEXP_CORE wxTreeListCtrl : public wxCompositeWindow< wxNavigationEnabled<wxWindow> >, public wxWithImages
{
public:
...
private:
  ...
  int DoInsertColumn(const wxString& title, int pos, int width, wxAlignment align, int flags);
    // Common part of {Append,Insert,Prepend}Item().
  wxTreeListItem DoInsertItem(wxTreeListItem parent, wxTreeListItem previous, const wxString& text, int imageClosed, int imageOpened, wxClientData* data);
...
typedef void (*wxTreeListEventFunction) (wxTreeListEvent&);
