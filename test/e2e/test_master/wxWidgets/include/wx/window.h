...
// global variables
// ----------------------------------------------------------------------------
WXDLLIMPEXP_CORE extern wxWindowList wxTopLevelWindows;
// declared here for compatibility only, main declaration is in wx/app.h
WXDLLIMPEXP_BASE extern wxList wxPendingDelete;
// ----------------------------------------------------------------------------
...
public:
  ...
  {
    DoSetVirtualSize(size.x, size.y);
  }
  ...
  virtual bool ShowWithEffect(wxShowEffect, unsigned = 0)
  {
    return Show();
  }
  virtual bool HideWithEffect(wxShowEffect, unsigned = 0)
  {
    return Hide();
  }
  ...
  {
    return (m_windowStyle & flag) != 0;
  }
  virtual bool IsRetained() const
  {
    return HasFlag(wxRETAINED);
  }
  ...
  {
    return AcceptsFocus() && CanBeFocused();
  }
  ...
  {
    return DoGetSibling(OrderBefore);
  }
  wxWindow* GetNextSibling() const
  {
    return DoGetSibling(OrderAfter);
  }
  ...
  void SetPreviousHandler(wxEvtHandler* handler) override;
protected:
...
#  endif
  ...
  {
    return FromDIP(sz, this);
  }
  wxPoint FromDIP(const wxPoint& pt) const
  {
    return FromDIP(pt, this);
  }
  int FromDIP(int d) const
  {
    return FromDIP(d, this);
  }
  ...
  {
    return ToDIP(sz, this);
  }
  wxPoint ToDIP(const wxPoint& pt) const
  {
    return ToDIP(pt, this);
  }
...
#  endif
  ...
  {
    return GetBorder(GetWindowStyleFlag());
  }
...
#  endif
  ...
  virtual bool ScrollLines(int)
  {
    return false;
  }
  virtual bool ScrollPages(int)
  {
    return false;
  }
...
#  else 
  ...
  void SetHelpTextForId(const wxString&)
  {
  }
#  endif
    // tooltips
...
protected:
  ...
  bool CreateBase(wxWindowBase* parent, wxWindowID winid, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = 0, const wxValidator& validator = wxDefaultValidator, const wxString& name = wxASCII_STR(wxPanelNameStr));
  bool CreateBase(wxWindowBase* parent, wxWindowID winid, const wxPoint& pos, const wxSize& size, long style, const wxString& name);
  ...
  {
    return w == wxDefaultCoord ? 20 : w;
  }
  static int HeightDefault(int h)
  {
    return h == wxDefaultCoord ? 20 : h;
  }
  ...
  wxSize m_bestSizeCache;
    ...
    // text extent
  virtual void DoGetTextExtent(const wxString& string, int* x, int* y, int* descent = NULL, int* externalLeading = NULL, const wxFont* font = NULL) const = 0;
...
/* static */
inline wxSize wxWindowBase::FromDIP(const wxSize& sz, const wxWindowBase*)
{
  return sz;
}
/* static */
inline wxSize wxWindowBase::ToDIP(const wxSize& sz, const wxWindowBase*)
{
  return sz;
}
#endif
...
// position.
WXDLLIMPEXP_CORE extern wxWindow* wxFindWindowAtPointer(wxPoint& pt);
// Get the current mouse position.
WXDLLIMPEXP_CORE extern wxPoint wxGetMousePosition();
...
{
public:
  wxWindowAccessible(wxWindow* win)
    : wxAccessible(win)
  {
    if (win)
    {
      win->SetAccessible(this);
    }
  }
