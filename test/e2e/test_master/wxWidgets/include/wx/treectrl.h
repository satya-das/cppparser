...
public:
 ...
 return m_imageListState;
  ...
  virtual int GetItemImage(const wxTreeItemId& item, wxTreeItemIcon which = wxTreeItemIcon_Normal) const = 0;
  ...
  virtual void SetItemImage(const wxTreeItemId& item, int image, wxTreeItemIcon which = wxTreeItemIcon_Normal) = 0;
  ...
  virtual void SetItemDropHighlight(const wxTreeItemId& item, bool highlight = true) = 0;
        // set the items text colour
  virtual void SetItemTextColour(const wxTreeItemId& item, const wxColour& col) = 0;
        // set the items background colour
  virtual void SetItemBackgroundColour(const wxTreeItemId& item, const wxColour& col) = 0;
 ...
 return ItemHasChildren(item);
  ...
  virtual size_t GetChildrenCount(const wxTreeItemId& item, bool recursively = true) const = 0;
    // navigation
    // ----------
  ...
  virtual wxTreeItemId GetFirstChild(const wxTreeItemId& item, wxTreeItemIdValue& cookie) const = 0;
        // get the next child
  virtual wxTreeItemId GetNextChild(const wxTreeItemId& item, wxTreeItemIdValue& cookie) const = 0;
  ...
  virtual wxTreeItemId AddRoot(const wxString& text, int image = -1, int selImage = -1, wxTreeItemData* data = NULL) = 0;
 ...
 SelectItem(item, false);
  ...
  virtual wxTextCtrl* EditLabel(const wxTreeItemId& item, wxClassInfo* textCtrlClass = wxCLASSINFO(wxTextCtrl)) = 0;
        // returns the same pointer as StartEdit() if the item is being edited,
        // NULL otherwise (it's assumed that no more than one item may be
        // edited simultaneously)
  virtual wxTextCtrl* GetEditControl() const = 0;
        // end editing and accept or discard the changes to item label
  virtual void EndEditLabel(const wxTreeItemId& item, bool discardChanges = false) = 0;
...
protected:
  ...
  virtual void DoSetItemState(const wxTreeItemId& item, int state) = 0;
    ...
    // it to the end
  virtual wxTreeItemId DoInsertItem(const wxTreeItemId& parent, size_t pos, const wxString& text, int image, int selImage, wxTreeItemData* data) = 0;
  ...
  virtual wxTreeItemId DoInsertAfter(const wxTreeItemId& parent, const wxTreeItemId& idPrevious, const wxString& text, int image = -1, int selImage = -1, wxTreeItemData* data = NULL) = 0;
  ...
  virtual wxTreeItemId DoTreeHitTest(const wxPoint& point, int& flags) const = 0;
  wxImageList* m_imageListNormal, *m_imageListState;
