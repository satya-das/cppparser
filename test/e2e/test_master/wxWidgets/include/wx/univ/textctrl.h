...
public:
  ...
  bool PositionToXY(wxTextPos pos, wxTextCoord* x, wxTextCoord* y) const override;
  ...
  wxTextCtrlHitTestResult HitTestLine(const wxString& line, wxCoord x, wxTextCoord* colOut) const;
...
protected:
  ...
  wxString GetTextToShow(const wxString& text) const;
    ...
    // start of line) is
  wxTextCoord GetRowInLine(wxTextCoord line, wxTextCoord col, wxTextCoord* colRowStart = NULL) const;
    // find the number of characters of a line before it wraps
    // (and optionally also the real width of the line)
  size_t GetPartOfWrappedLine(const wxChar* text, wxCoord* widthReal = NULL) const;
  ...
  bool GetSelectedPartOfLine(wxTextCoord line, wxTextPos* start, wxTextPos* end) const;
  ...
  wxTextCtrlHitTestResult HitTest2(wxCoord y, wxCoord x1, wxCoord x2, wxTextCoord* row, wxTextCoord* colStart, wxTextCoord* colEnd, wxTextCoord* colRowStart, bool devCoords = true) const;
    // HitTest() version which takes the logical text coordinates and not the
    // device ones
  wxTextCtrlHitTestResult HitTestLogical(const wxPoint& pos, wxTextCoord* col, wxTextCoord* row) const;
  ...
  bool GetLineAndRow(wxTextCoord row, wxTextCoord* line, wxTextCoord* rowInLine) const;
  ...
  struct wxTextSingleLineData& SData()
  {
    return *m_data.sdata;
  }
  struct wxTextMultiLineData& MData()
  {
    return *m_data.mdata;
  }
  struct wxTextWrappedData& WData()
  {
    return *m_data.wdata;
  }
  const wxTextSingleLineData& SData() const
  {
    return *m_data.sdata;
  }
  const wxTextMultiLineData& MData() const
  {
    return *m_data.mdata;
  }
  const wxTextWrappedData& WData() const
  {
    return *m_data.wdata;
  }
...
private:
  ...
  wxString m_value;
    // current position
  wxTextPos m_curPos;
  wxTextCoord m_curCol, m_curRow;
    // last position (only used by GetLastPosition())
  wxTextPos m_posLast;
    // max text line length
  unsigned long m_maxLength;
    // selection
  wxTextPos m_selAnchor, m_selStart, m_selEnd;
    // flags
  bool m_isModified, m_isEditable, m_hasCaret, m_wrapLines;
