...
public:
  ...
  public:
    ...
    compatibility_iterator(const ListType* li, iterator i)
      : m_iter(i)
      , m_list(const_cast<ListType*>(li))
    {
    }
    compatibility_iterator* operator->()
    {
      return this;
    }
    const compatibility_iterator* operator->() const
    {
      return this;
    }
    bool operator==(const compatibility_iterator& i) const
    {
      wxASSERT_MSG(m_list && i.m_list, "comparing invalid iterators is illegal");
    ...
    int IndexOf() const
    {
      return *this ? std::distance(m_list->begin(), m_iter) : wxNOT_FOUND;
    }
  };
  wxDList()
    : m_destroy(false)
  ...
  {
    return compatibility_iterator(this, std::find(const_cast<ListType*>(this)->begin(), const_cast<ListType*>(this)->end(), e));
  }
  bool IsEmpty() const
  {
    return this->empty();
  ...
  {
    return compatibility_iterator(this, this->insert(Item(idx).m_iter, e));
  }
...
public:
  ...
  public:
    ...
    T** GetDataPtr() const
    {
      return &(const_cast<nodetype*>(this)->m_data);
    }
    void SetData(T* data)
    {
      m_data = data;
  ...
  {
  public:
    compatibility_iterator(nodetype* ptr = NULL)
      : m_ptr(ptr)
    {
    }
...
private:
  ...
  size_t m_count;
  bool m_destroy;
  nodetype* m_nodeFirst, *m_nodeLast;
public:
  wxDList()
  {
    Init();
  ...
  {
    wxASSERT_MSG(!list.m_destroy, "copying list which owns it's elements is a bad idea");
    Clear();
  ...
  {
    wxCHECK_MSG(!position || position->m_list == this, NULL, "can't insert before a node from another list");
  ...
  nodetype* DetachNode(nodetype* node)
  {
    ...
    wxCHECK_MSG(node->m_list == this, NULL, "detaching node which is not from this list");
        // update the list
    nodetype** prevNext = node->GetPrevious() ? &node->GetPrevious()->m_next : &m_nodeFirst;
    nodetype** nextPrev = node->GetNext() ? &node->GetNext()->m_previous : &m_nodeLast;
  ...
  iterator begin()
  {
    return iterator(GetFirst(), GetLast());
  }
  ...
  iterator end()
  {
    return iterator(NULL, GetLast());
  }
  const_iterator end() const
  {
    return const_iterator(NULL, GetLast());
  }
  ...
  {
    return reverse_iterator(NULL, GetFirst());
  }
