...
#    else 
  ...
  LPLOGFONTW GetPtr()
  {
    return UseLOGFONTW() ? &m_lfW : reinterpret_cast<LPLOGFONTW>(&m_lfA);
  }
  ...
  const LOGFONTA& GetLOGFONT()
  {
    if (UseLOGFONTW())
    {
      ...
      WideCharToMultiByte(CP_ACP, 0, m_lfW.lfFaceName, -1, m_lfA.lfFaceName, sizeof(m_lfA.lfFaceName), NULL, NULL);
    }
    return m_lfA;
  }
private:
