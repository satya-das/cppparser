...
public:
  ...
  wxMSWFileSystemWatcher(const wxFileName& path, int events = wxFSW_EVENT_ALL);
  ...
  bool AddTree(const wxFileName& path, int events = wxFSW_EVENT_ALL, const wxString& filter = wxEmptyString) override;
protected:
  bool Init();
};
#  endif
