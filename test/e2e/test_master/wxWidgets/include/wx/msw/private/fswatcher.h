...
public:
  ...
  bool ScheduleForRemoval(const wxSharedPtr<wxFSWatchEntryMSW>& watch)
  {
    ...
    wxCHECK_MSG(it != m_watches.end(), false, "Can't remove a watch we don't use");
  ...
  {
    wxCHECK_MSG(m_iocp != INVALID_HANDLE_VALUE, Status_Error, "Invalid IOCP object");
    wxCHECK_MSG(count && watch && overlapped, Status_Error, "Output parameters can't be NULL");
...
protected:
  ...
  static wxString FileNotifyInformationToString(const FILE_NOTIFY_INFORMATION& e);
  static wxFileName GetEventPath(const wxFSWatchEntryMSW& watch, const FILE_NOTIFY_INFORMATION& e);
