...
public:
  ...
  bool Save(const Serializer& ser) const override
  {
    ...
    if (!ser.SaveField(wxPERSIST_TLW_X, rc.left) || !ser.SaveField(wxPERSIST_TLW_Y, rc.top))
    {
      return false;
    }
    if (!ser.SaveField(wxPERSIST_TLW_W, rc.right - rc.left) || !ser.SaveField(wxPERSIST_TLW_H, rc.bottom - rc.top))
    {
      return false;
    }
    ...
    if (!ser.SaveField(wxPERSIST_TLW_MAX_X, pt.x) || !ser.SaveField(wxPERSIST_TLW_MAX_Y, pt.y))
    {
      return false;
    }
  ...
  bool Restore(Serializer& ser) override
  {
        // Normal position and size.
    wxRect r;
    if (!ser.RestoreField(wxPERSIST_TLW_X, &r.x) || !ser.RestoreField(wxPERSIST_TLW_Y, &r.y) || !ser.RestoreField(wxPERSIST_TLW_W, &r.width) || !ser.RestoreField(wxPERSIST_TLW_H, &r.height))
    {
      return false;
    }
    ...
    {
      if (ser.RestoreField(wxPERSIST_TLW_ICONIZED, &tmp) && tmp)
      {
        show = SW_MINIMIZE;
      }
      else 
