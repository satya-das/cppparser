...
// ----------------------------------------------------------------------------
class WXDLLIMPEXP_NET wxProtocol : public wxSocketClient
{
public:
  wxProtocol();
  virtual ~wxProtocol();
  bool Reconnect();
  virtual bool Connect(const wxString&)
  {
    return false;
  }
  bool Connect(const wxSockAddress& addr, bool = true) override
  {
...
{
public:
  wxProtoInfo(const wxChar* name, const wxChar* serv_name, const bool need_host1, wxClassInfo* info);
protected:
  wxProtoInfo* next;
