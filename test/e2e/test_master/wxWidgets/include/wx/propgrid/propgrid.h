...
WXDLLIMPEXP_PROPGRID extern const char wxPropertyGridNameStr[];
class wxPGComboBox;
#    if  wxUSE_STATUSBAR
...
#    if  wxUSE_VALIDATORS
  wxVector<wxValidator*> m_arrValidators;
#    endif
...
#    endif
  ...
  int m_extraStyle;
  int m_warnings;
  int HasExtraStyle(int style) const
  {
    return (m_extraStyle & style);
  }
};
WXDLLIMPEXP_PROPGRID extern wxPGGlobalVarsClass* wxPGGlobalVars;
...
// possible moment.
class WXDLLIMPEXP_PROPGRID wxPropertyGrid : public wxScrolled<wxControl>, public wxPropertyGridInterface
{
...
public:
  ...
  wxPropertyGrid();
    ...
    // the wxWindow.
  wxPropertyGrid(wxWindow* parent, wxWindowID id = wxID_ANY, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = wxPG_DEFAULT_STYLE, const wxString& name = wxASCII_STR(wxPropertyGridNameStr));
    // Destructor
  ...
  bool Create(wxWindow* parent, wxWindowID id = wxID_ANY, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, long style = wxPG_DEFAULT_STYLE, const wxString& name = wxASCII_STR(wxPropertyGridNameStr));
  ...
  static wxPGEditor* DoRegisterEditorClass(wxPGEditor* editorClass, const wxString& editorName, bool noDefCheck = false);
  ...
  static wxString& ExpandEscapeSequences(wxString& dst_str, const wxString& src_str);
    // Converts newlines, tabs, etc. in src_str to escape
    // sequences, and copies result to dst_str.
  static wxString& CreateEscapeSequences(wxString& dst_str, const wxString& src_str);
  ...
  {
    return (event.GetEventType() == wxEVT_BUTTON) && (m_wndSecId == event.GetId());
  }
...
protected:
  ...
  virtual bool PerformValidation(wxPGProperty* p, wxVariant& pendingValue, int flags = SendEvtChanging);
public:
  ...
  void SetScrollbars(int pixelsPerUnitX, int pixelsPerUnitY, int noUnitsX, int noUnitsY, int xPos, int yPos, bool noRefresh) override;
protected:
...
#    endif
  ...
  void DrawItems(wxDC& dc, unsigned int topItemY, unsigned int bottomItemY, const wxRect* itemsRect = NULL);
  ...
  void DoSetSplitterPosition(int newxpos, int splitterIndex = 0, int flags = wxPG_SPLITTER_REFRESH);
  ...
  void SetEditorAppearance(const wxPGCell& cell, bool unspecified = false);
  ...
  bool SendEvent(wxEventType eventType, wxPGProperty* p, wxVariant* pValue = NULL, unsigned int selFlags = wxPG_SEL_NOVALIDATE, unsigned int column = 1);
...
typedef void (*wxPropertyGridEventFunction) (wxPropertyGridEvent&);
