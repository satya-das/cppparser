...
protected:
  ...
  static void DeleteNodes(size_t buckets, _wxHashTable_NodeBase** table, NodeDtor dtor);
  static _wxHashTable_NodeBase* GetFirstNode(size_t buckets, _wxHashTable_NodeBase** table)
  {
 ...
 prime_count = 31
  ...
  static void CopyHashTable(_wxHashTable_NodeBase** srcTable, size_t srcBuckets, _wxHashTableBase2* dst, _wxHashTable_NodeBase** dstTable, BucketFromNode func, ProcessNode proc);
...
#      if  (!defined SIZEOF_LONG_LONG && SIZEOF_LONG == 4) \
  ...
  {
    return longHash(wx_truncate_cast(long, x)) ^ longHash(wx_truncate_cast(long, x >> (sizeof(long) * 8)));
  }
