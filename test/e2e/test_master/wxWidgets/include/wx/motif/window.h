...
protected:
  ...
  void DoSetSizeIntr(int x, int y, int width, int height, int sizeFlags, bool fromCtor);
...
protected:
  ...
  bool AttachWidget(wxWindow* parent, WXWidget mainWidget, WXWidget formWidget, int x, int y, int width, int height);
...
protected:
  ...
  virtual void DoGetTextExtent(const wxString& string, int* x, int* y, int* descent = NULL, int* externalLeading = NULL, const wxFont* font = NULL) const;
