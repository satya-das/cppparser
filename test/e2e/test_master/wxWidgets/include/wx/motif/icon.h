...
public:
  ...
  virtual ~wxIcon();
  bool LoadFile(const wxString& name, wxBitmapType type, int desiredWidth, int desiredHeight);
    // unhide the base class version
  virtual bool LoadFile(const wxString& name, wxBitmapType flags = wxICON_DEFAULT_TYPE)
