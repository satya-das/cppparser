...
WXDLLIMPEXP_BASE extern const wxChar* wxEmptyString;
#  if  wxUSE_UNICODE_UTF8
// FIXME-UTF8: we should have only one wxEmptyString
WXDLLIMPEXP_BASE extern const wxStringCharType* wxEmptyStringImpl;
#  endif
// ----------------------------------------------------------------------------
...
{
  int nRefs;
  size_t nDataLength, nAllocLength;
  // mimics declaration 'wxStringCharType data[nAllocLength]'
  wxStringCharType* data()
  {
    return reinterpret_cast<wxStringCharType*>(this + 1);
  }
  const wxStringCharType* data() const
  {
    return reinterpret_cast<const wxStringCharType*>(this + 1);
  }
...
#    endif
  ...
  const wxStringCharType* c_str() const
  {
    return m_pchData;
  }
  const wxStringCharType* data() const
  ...
  wxStringImpl& replace(size_t nStart, size_t nLen, const wxStringImpl& str, size_t nStart2, size_t nLen2)
  {
    return replace(nStart, nLen, str.substr(nStart2, nLen2));
  }
    // replaces the substring with first nCount chars of sz
  wxStringImpl& replace(size_t nStart, size_t nLen, const wxStringCharType* sz, size_t nCount);
  wxStringImpl& replace(iterator first, iterator last, const_pointer s)
  {
    return replace(first - begin(), last - first, s);
  }
  wxStringImpl& replace(iterator first, iterator last, const_pointer s, size_type n)
  {
  ...
  wxStringImpl& replace(iterator first, iterator last, const_iterator first1, const_iterator last1)
  {
  ...
  size_t rfind(const wxStringImpl& str, size_t nStart = npos) const;
    // as find, but from the end
  size_t rfind(const wxStringCharType* sz, size_t nStart = npos, size_t n = npos) const;
    // as find, but from the end
