...
#  endif
  ...
  public:
    ...
    {
      float32x4x4_t rgba = {{r.fVec, g.fVec, b.fVec, a.fVec}};
      vst4q_f32((float*) dst, rgba);
    }
    AI SkNx invert() const
    {
      float32x4_t est0, est1;
      return est1;
    }
    AI SkNx operator -() const
    {
...
// It'd make conversion to Sk4b one step simpler.
  ...
  public:
    ...
    AI static void Store4(void* dst, const SkNx& r, const SkNx& g, const SkNx& b, const SkNx& a)
    {
      uint16x4x4_t rgba = {{r.fVec, g.fVec, b.fVec, a.fVec}};
      ...
      union 
{
  ...
  AI Sk16b SkNx_cast<uint8_t, float>(const Sk16f& src)
  {
    ...
    SkNx_split(ab, &a, &b);
    SkNx_split(cd, &c, &d);
    return vuzpq_u8(vuzpq_u8((uint8x16_t) vcvtq_u32_f32(a.fVec), (uint8x16_t) vcvtq_u32_f32(b.fVec)).val[0], vuzpq_u8((uint8x16_t) vcvtq_u32_f32(c.fVec), (uint8x16_t) vcvtq_u32_f32(d.fVec)).val[0]).val[0];
  }
  template <>
