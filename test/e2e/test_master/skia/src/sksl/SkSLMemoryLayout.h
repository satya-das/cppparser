    ...
    size_t alignment(const Type& type) const
    {
      ...
      switch(type.kind())
      {
        ...
        case Type::kMatrix_Kind:
          return this->roundUpIfNeeded(vector_alignment(this->size(type.componentType()), type.rows()));
    ...
    size_t size(const Type& type) const
    {
      switch(type.kind())
      {
        ...
        case Type::kStruct_Kind:
          ...
          SkASSERT(!type.fields().size() || (0 == alignment % this->alignment(*type.fields()[0].fType)));
