...
default:
    ...
    Layout(int flags, int location, int offset, int binding, int index, int set, int builtin, int inputAttachmentIndex, Format format, Primitive primitive, int maxVertices, int invocations, StringFragment when, Key key, CType ctype)
      : fFlags(flags)
      , fLocation(location)
      , fOffset(offset)
      , fBinding(binding)
