 ...
 */
  static const Type& get_type(const Context& context, Expression& value, size_t count)
  {
    ...
    else 
    {
      ...
      {
        if (base == *context.fDouble_Type)
        {
          switch(count)
          {
            case 2:
              return *context.fDouble2_Type;
            case 3:
              return *context.fDouble3_Type;
            case 4:
              return *context.fDouble4_Type;
          }
        }
        else 
        {
          if (base == *context.fInt_Type)
          {
            switch(count)
            {
              case 2:
          ...
          else 
          {
            ...
            {
              if (base == *context.fByte_Type)
              {
                switch(count)
                {
                  case 2:
                    return *context.fByte2_Type;
                  case 3:
                    return *context.fByte3_Type;
                  case 4:
                    return *context.fByte4_Type;
                }
              }
              else 
              {
                if (base == *context.fUInt_Type)
                {
                  switch(count)
                  {
                    case 2:
                      return *context.fUInt2_Type;
                    case 3:
                      return *context.fUInt3_Type;
                    case 4:
                      return *context.fUInt4_Type;
                  }
                }
                else 
                {
                  if (base == *context.fUShort_Type)
                  {
                    switch(count)
                    {
                      case 2:
                        return *context.fUShort2_Type;
                      case 3:
                        return *context.fUShort3_Type;
                      case 4:
                        return *context.fUShort4_Type;
                    }
                  }
                  else 
                  {
                    if (base == *context.fUByte_Type)
                    {
                      switch(count)
                      {
                        case 2:
                          return *context.fUByte2_Type;
                        case 3:
                          return *context.fUByte3_Type;
                        case 4:
                          return *context.fUByte4_Type;
                      }
                    }
                    else 
                    {
                      if (base == *context.fBool_Type)
                      {
                        switch(count)
                        {
                          case 2:
    ...
    std::unique_ptr<Expression> constantPropagate(const IRGenerator& irGenerator, const DefinitionMap& definitions) override
    {
      if (fBase->fKind == Expression::kConstructor_Kind && fBase->isConstant())
      {
        ...
        if (fType.isInteger())
        {
          ...
          return std::unique_ptr<Expression>(new IntLiteral(irGenerator.fContext, -1, value));
        }
        else 
        {
          if (fType.isFloat())
          {
            ...
            return std::unique_ptr<Expression>(new FloatLiteral(irGenerator.fContext, -1, value));
          }
        }
      }
      return nullptr;
