 ...
 */
  ...
  {
    TernaryExpression(int offset, std::unique_ptr<Expression> test, std::unique_ptr<Expression> ifTrue, std::unique_ptr<Expression> ifFalse)
    ...
    {
      return std::unique_ptr<Expression>(new TernaryExpression(fOffset, fTest->clone(), fIfTrue->clone(), fIfFalse->clone()));
    }
    String description() const override
    {
      return "(" + fTest->description() + " ? " + fIfTrue->description() + " : " + fIfFalse->description() + ")";
    }
