...
struct SkDCubic
{
  ...
  {
    return fPts[0].approximatelyEqual(fPts[1]) && fPts[0].approximatelyEqual(fPts[2]) && fPts[0].approximatelyEqual(fPts[3]);
  }
...
// add debug only global pointer so asserts can be skipped by fuzzers
  const SkDCubic& set(const SkPoint pts[kPointCount], SkOpGlobalState* state = nullptr)
  {
    fPts[0] = pts[0];
    fPts[1] = pts[1];
    fPts[2] = pts[2];
