...
struct SkImageFilterCacheKey
{
    ...
    , fSrcSubset(srcSubset)
  {
    ...
    static_assert(sizeof(SkImageFilterCacheKey) == sizeof(uint32_t) + sizeof(SkMatrix) + sizeof(SkIRect) + sizeof(uint32_t) + 4 * sizeof(int32_t), "image_filter_key_tight_packing");
    fMatrix.getType();
...
public:
  ...
  virtual bool get(const SkImageFilterCacheKey& key, skif::FilterResult<For::kOutput>* result) const = 0;
    // 'filter' is included in the caching to allow the purging of all of an image filter's cached
    // results when it is destroyed.
  virtual void set(const SkImageFilterCacheKey& key, const SkImageFilter* filter, const skif::FilterResult<For::kOutput>& result) = 0;
