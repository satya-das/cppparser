...
public:
  ...
  virtual SkMask::Format getFormat() const = 0;
    ...
    */
  virtual bool filterMask(SkMask* dst, const SkMask& src, const SkMatrix&, SkIPoint* margin) const = 0;
#  if  SK_SUPPORT_GPU
    /**
  ...
  bool hasFragmentProcessor() const;
     ...
     */
  virtual bool canFilterMaskGPU(const GrShape&, const SkIRect& devSpaceShapeBounds, const SkIRect& clipBounds, const SkMatrix& ctm, SkIRect* maskRect) const;
    /**
     *  Try to directly render the mask filter into the target. Returns true if drawing was
     *  successful. If false is returned then paint is unmodified.
     */
  virtual bool directFilterMaskGPU(GrRecordingContext*, GrRenderTargetContext*, GrPaint&& paint, const GrClip&, const SkMatrix& viewMatrix, const GrShape& shape) const;
    /**
     ...
     */
  virtual sk_sp<GrTextureProxy> filterMaskGPU(GrRecordingContext*, sk_sp<GrTextureProxy> srcProxy, GrColorType srcColorType, SkAlphaType srcAlphaType, const SkMatrix& ctm, const SkIRect& maskRect) const;
#  endif
    /**
