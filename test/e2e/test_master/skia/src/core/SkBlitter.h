...
public:
  ...
  void blitRegion(const SkRegion& clip);
     ...
     */
  static SkBlitter* Choose(const SkPixmap& dst, const SkMatrix& matrix, const SkPaint& paint, SkArenaAlloc*, bool drawCoverage = false);
  static SkBlitter* ChooseSprite(const SkPixmap& dst, const SkPaint&, const SkPixmap& src, int left, int top, SkArenaAlloc*);
    ///@}
...
public:
  ...
  void blitV(int x, int y, int height, SkAlpha alpha) override;
  void blitRect(int x, int y, int width, int height) override;
  virtual void blitAntiRect(int x, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha) override;
  void blitMask(const SkMask&, const SkIRect& clip) override;
  const SkPixmap* justAnOpaqueColor(uint32_t* value) override;
...
public:
  SkPairBlitter(SkBlitter* a, SkBlitter* b)
    : fA(a)
    , fB(b)
  {
  }
  ...
  void blitAntiH2(int x, int y, U8CPU a0, U8CPU a1) override
  {
    SHARD(blitAntiH2(x, y, a0, a1))
  }
  void blitAntiV2(int x, int y, U8CPU a0, U8CPU a1) override
  {
    SHARD(blitAntiV2(x, y, a0, a1))
  }
};
#  undef SHARD
#endif
