...
public:
  ...
  typename std::enable_if<std::is_base_of<GrGpuResource, T>::value, sk_sp<T>>::type findByUniqueKey(const GrUniqueKey& key)
  {
     ...
     */
  sk_sp<GrTexture> createApproxTexture(const GrSurfaceDesc& desc, const GrBackendFormat& format, GrRenderable renderable, int renderTargetSampleCnt, GrProtected isProtected);
    /** Create an exact fit texture with no initial data to upload. */
  sk_sp<GrTexture> createTexture(const GrSurfaceDesc& desc, const GrBackendFormat& format, GrRenderable renderable, int renderTargetSampleCnt, GrMipMapped mipMapped, SkBudgeted budgeted, GrProtected isProtected);
    /**
     ...
     */
  sk_sp<GrTexture> createTexture(const GrSurfaceDesc& desc, const GrBackendFormat& format, GrColorType colorType, GrRenderable renderable, int renderTargetSampleCnt, SkBudgeted budgeted, GrProtected isProtected, const GrMipLevel texels[], int mipLevelCount);
    /**
     * Create a potentially loose fit texture with the provided data. The color type must be valid
     * for the format and also describe the texel data. This will ensure any conversions that
     * need to get applied to the data before upload are applied.
     */
  sk_sp<GrTexture> createTexture(const GrSurfaceDesc& desc, const GrBackendFormat& format, GrColorType srcColorType, GrRenderable renderable, int renderTargetSampleCnt, SkBudgeted budgeted, SkBackingFit fit, GrProtected isProtected, const GrMipLevel& mipLevel);
    /**
     ...
     */
  sk_sp<GrTexture> createCompressedTexture(int width, int height, const GrBackendFormat&, SkImage::CompressionType, SkBudgeted, SkData* data);
    ///////////////////////////////////////////////////////////////////////////
     ...
     */
  sk_sp<GrTexture> wrapBackendTexture(const GrBackendTexture& tex, GrColorType, GrWrapOwnership, GrWrapCacheable, GrIOType);
    /**
     ...
     * to the texture.
     */
  sk_sp<GrTexture> wrapRenderableBackendTexture(const GrBackendTexture& tex, int sampleCnt, GrColorType, GrWrapOwnership, GrWrapCacheable);
    /**
  ...
  static const uint32_t kMinScratchTextureSize;
     ...
     */
  sk_sp<const GrGpuBuffer> findOrMakeStaticBuffer(GrGpuBufferType intendedType, size_t size, const void* data, const GrUniqueKey& key);
    /**
     ...
     */
  sk_sp<const GrGpuBuffer> findOrCreatePatternedIndexBuffer(const uint16_t* pattern, int patternSize, int reps, int vertCount, const GrUniqueKey& key)
  {
  ...
  sk_sp<GrPath> createPath(const SkPath&, const GrStyle&);
     ...
     * @return the buffer if successful, otherwise nullptr.
     */
  sk_sp<GrGpuBuffer> createBuffer(size_t size, GrGpuBufferType intendedType, GrAccessPattern, const void* data = nullptr);
    /**
  ...
  bool attachStencilAttachment(GrRenderTarget* rt, int numStencilSamples);
      ...
      * @return GrRenderTarget object or NULL on failure.
      */
  sk_sp<GrRenderTarget> wrapBackendTextureAsRenderTarget(const GrBackendTexture&, int sampleCnt, GrColorType);
    /**
...
private:
  ...
  sk_sp<GrTexture> refScratchTexture(const GrSurfaceDesc&, const GrBackendFormat&, GrRenderable, int renderTargetSampleCnt, GrMipMapped, GrProtected);
    /*
     * Try to find an existing scratch texture that exactly matches 'desc'. If successful
     * update the budgeting accordingly.
     */
  sk_sp<GrTexture> getExactScratch(const GrSurfaceDesc&, const GrBackendFormat&, GrRenderable, int renderTargetSampleCnt, SkBudgeted, GrMipMapped, GrProtected);
  ...
  GrColorType prepareLevels(const GrBackendFormat& format, GrColorType, const SkISize& baseSize, const GrMipLevel texels[], int mipLevelCount, TempLevels*, TempLevelDatas*) const;
