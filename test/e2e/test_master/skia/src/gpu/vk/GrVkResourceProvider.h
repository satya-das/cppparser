...
public:
  ...
  void init();
  GrVkPipeline* createPipeline(int numColorSamples, const GrPrimitiveProcessor& primProc, const GrPipeline& pipeline, const GrStencilSettings& stencil, GrSurfaceOrigin, VkPipelineShaderStageCreateInfo* shaderStageInfo, int shaderStageCount, GrPrimitiveType primitiveType, VkRenderPass compatibleRenderPass, VkPipelineLayout layout);
  ...
  const GrVkRenderPass* findCompatibleRenderPass(const GrVkRenderTarget& target, CompatibleRPHandle* compatibleHandle = nullptr);
  ...
  const GrVkRenderPass* findRenderPass(const GrVkRenderTarget& target, const GrVkRenderPass::LoadStoreOps& colorOps, const GrVkRenderPass::LoadStoreOps& stencilOps, CompatibleRPHandle* compatibleHandle = nullptr);
    // The CompatibleRPHandle must be a valid handle previously set by a call to findRenderPass or
    // findCompatibleRenderPass.
  const GrVkRenderPass* findRenderPass(const CompatibleRPHandle& compatibleHandle, const GrVkRenderPass::LoadStoreOps& colorOps, const GrVkRenderPass::LoadStoreOps& stencilOps);
  ...
  void checkCommandBuffers();
    ...
    // finishedProc.
  void addFinishedProcToActiveCommandBuffers(GrGpuFinishedProc finishedProc, GrGpuFinishedContext finishedContext);
  ...
  GrVkSampler* findOrCreateCompatibleSampler(const GrSamplerState&, const GrVkYcbcrConversionInfo& ycbcrInfo);
  ...
  GrVkPipelineState* findOrCreateCompatiblePipelineState(GrRenderTarget*, int numSamples, GrSurfaceOrigin, const GrPipeline&, const GrPrimitiveProcessor&, const GrTextureProxy* const primProcProxies[], GrPrimitiveType, VkRenderPass compatibleRenderPass);
  void getSamplerDescriptorSetHandle(VkDescriptorType type, const GrVkUniformHandler&, GrVkDescriptorSetManager::Handle* handle);
  void getSamplerDescriptorSetHandle(VkDescriptorType type, const SkTArray<uint32_t>& visibilities, GrVkDescriptorSetManager::Handle* handle);
...
private:
  ...
  public:
    ...
    void abandon();
    void release();
    GrVkPipelineState* refPipelineState(GrRenderTarget*, int numSamples, GrSurfaceOrigin, const GrPrimitiveProcessor&, const GrTextureProxy* const primProcProxies[], const GrPipeline&, GrPrimitiveType, VkRenderPass compatibleRenderPass);
  private:
    struct Entry;
    struct DescHash
    {
