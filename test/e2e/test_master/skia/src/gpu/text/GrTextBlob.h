...
default:
  ...
  static const size_t kGrayTextDFPerspectiveVASize = sizeof(SkPoint3) + sizeof(GrColor) + sizeof(SkIPoint16);
  ...
  void initReusableBlob(SkColor luminanceColor, const SkMatrix& viewMatrix, SkScalar x, SkScalar y)
  {
    ...
    struct 
{
  ...
  struct Run
  {
    ...
    void switchSubRunIfNeededAndAppendGlyph(GrGlyph* glyph, const sk_sp<GrTextStrike>& strike, const SkRect& destRect, bool needsTransform);
        // Used when the glyph in the cache has the CTM already applied, therefore no transform
        // is needed during rendering.
    void appendDeviceSpaceGlyph(const sk_sp<GrTextStrike>& strike, const SkGlyph& skGlyph, SkPoint origin);
        // The glyph is oriented upright in the cache and needs to be transformed onto the screen.
    void appendSourceSpaceGlyph(const sk_sp<GrTextStrike>& strike, const SkGlyph& skGlyph, SkPoint origin, SkScalar textScale);
  ...
  std::unique_ptr<GrAtlasTextOp> makeOp(const SubRun& info, int glyphCount, uint16_t run, uint16_t subRun, const SkMatrix& viewMatrix, SkScalar x, SkScalar y, const SkIRect& clipRect, const SkPaint& paint, const SkPMColor4f& filteredColor, const SkSurfaceProps&, const GrDistanceFieldAdjustTable*, GrTextTarget*);
  ...
  void processDeviceMasks(SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec) override;
  void processSourcePaths(SkSpan<const SkGlyphPos> paths, const SkStrikeSpec& strikeSpec) override;
  void processDevicePaths(SkSpan<const SkGlyphPos> paths) override;
  void processSourceSDFT(SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec, const SkFont& runFont, SkScalar minScale, SkScalar maxScale, bool hasWCoord) override;
  void processSourceFallback(SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec, bool hasW) override;
  void processDeviceFallback(SkSpan<const SkGlyphPos> masks, const SkStrikeSpec& strikeSpec) override;
  struct StrokeInfo
  {
...
public:
     ...
     * SkGlyphCache.
     */
  VertexRegenerator(GrResourceProvider*, GrTextBlob*, int runIdx, int subRunIdx, const SkMatrix& viewMatrix, SkScalar x, SkScalar y, GrColor color, GrDeferredUploadTarget*, GrStrikeCache*, GrAtlasManager*, SkExclusiveStrikePtr*);
  struct Result
  {
        /**
