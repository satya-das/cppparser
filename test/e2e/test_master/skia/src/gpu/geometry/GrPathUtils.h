...
namespace GrPathUtils
{
  ...
  int worstCasePointCount(const SkPath&, int* subpaths, SkScalar tol);
 ...
 this->set(controlPts);     }
  ...
  void convertCubicToQuadsConstrainToTangents(const SkPoint p[4], SkScalar tolScale, SkPathPriv::FirstDirection dir, SkTArray<SkPoint, true>* quads);
  ...
  SkCubicType getCubicKLM(const SkPoint src[4], SkMatrix* klm, double t[2], double s[2]);
    ...
    //
    //            KLM.postScale(-1, -1)
  int chopCubicAtLoopIntersection(const SkPoint src[4], SkPoint dst[10], SkMatrix* klm, int* loopIndex);
