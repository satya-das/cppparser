...
public:
  static bool TestForPreservingPMConversions(GrContext* context)
  {
    ...
    const SkImageInfo ii = SkImageInfo::Make(kSize, kSize, kRGBA_8888_SkColorType, kPremul_SkAlphaType);
    auto readRTC = context->priv().makeDeferredRenderTargetContext(SkBackingFit::kExact, kSize, kSize, kColorType, nullptr);
    auto tempRTC = context->priv().makeDeferredRenderTargetContext(SkBackingFit::kExact, kSize, kSize, kColorType, nullptr);
    ...
    sk_sp<GrTextureProxy> dataProxy = proxyProvider->createTextureProxy(std::move(image), 1, SkBudgeted::kYes, SkBackingFit::kExact);
    if (!dataProxy)
    {
      return false;
    }
    ...
    GrPaint paint2;
    GrPaint paint3;
    std::unique_ptr<GrFragmentProcessor> pmToUPM(new GrConfigConversionEffect(PMConversion::kToUnpremul));
    std::unique_ptr<GrFragmentProcessor> upmToPM(new GrConfigConversionEffect(PMConversion::kToPremul));
