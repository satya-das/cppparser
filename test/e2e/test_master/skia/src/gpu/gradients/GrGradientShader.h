...
{
  std::unique_ptr<GrFragmentProcessor> MakeLinear(const SkLinearGradient& shader, const GrFPArgs& args);
  std::unique_ptr<GrFragmentProcessor> MakeRadial(const SkRadialGradient& shader, const GrFPArgs& args);
  std::unique_ptr<GrFragmentProcessor> MakeSweep(const SkSweepGradient& shader, const GrFPArgs& args);
  std::unique_ptr<GrFragmentProcessor> MakeConical(const SkTwoPointConicalGradient& shader, const GrFPArgs& args);
#  if  GR_TEST_UTILS
