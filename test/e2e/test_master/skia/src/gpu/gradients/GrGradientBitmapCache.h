...
public:
  ...
  void getGradient(const SkPMColor4f* colors, const SkScalar* positions, int count, SkColorType colorType, SkAlphaType alphaType, SkBitmap* bitmap);
private:
  SkMutex fMutex;
  int fEntryCount;
  ...
  void fillGradient(const SkPMColor4f* colors, const SkScalar* positions, int count, SkColorType colorType, SkBitmap* bitmap);
#  ifdef SK_DEBUG
  void validate() const;
#  else 
  void validate() const
