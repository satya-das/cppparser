...
public:
  ...
  void emitCode(GrGLSLUniformHandler* uniformHandler, const GrColorSpaceXform* colorSpaceXform, uint32_t visibility = kFragment_GrShaderFlag)
  {
    ...
    if (colorSpaceXform)
    {
      ...
      if (this->applySrcTF())
      {
        fSrcTFVar = uniformHandler->addUniformArray(visibility, kHalf_GrSLType, "SrcTF", kNumTransferFnCoeffs);
      }
      ...
      if (this->applyDstTF())
      {
        fDstTFVar = uniformHandler->addUniformArray(visibility, kHalf_GrSLType, "DstTF", kNumTransferFnCoeffs);
      }
    }
  }
