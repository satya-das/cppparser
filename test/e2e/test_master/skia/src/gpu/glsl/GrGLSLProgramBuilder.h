...
protected:
  explicit GrGLSLProgramBuilder(GrRenderTarget* renderTarget, int numSamples, GrSurfaceOrigin origin, const GrPrimitiveProcessor&, const GrTextureProxy* const primProcProxies[], const GrPipeline&, GrProgramDesc*);
...
private:
  ...
  SkString emitAndInstallFragProc(const GrFragmentProcessor&, int index, int transformedCoordVarsIdx, const SkString& input, SkString output, SkTArray<std::unique_ptr<GrGLSLFragmentProcessor>>*);
  void emitAndInstallXferProc(const SkString& colorIn, const SkString& coverageIn);
  SamplerHandle emitSampler(const GrTexture*, const GrSamplerState&, const GrSwizzle&, const char* name);
