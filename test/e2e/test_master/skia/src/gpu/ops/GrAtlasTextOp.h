...
public:
  ...
  static std::unique_ptr<GrAtlasTextOp> MakeBitmap(GrRecordingContext*, GrPaint&&, GrMaskFormat, int glyphCount, bool needsTransform);
  static std::unique_ptr<GrAtlasTextOp> MakeDistanceField(GrRecordingContext*, GrPaint&&, int glyphCount, const GrDistanceFieldAdjustTable*, bool useGammaCorrectDistanceTable, SkColor luminanceColor, const SkSurfaceProps&, bool isAntiAliased, bool useLCD);
...
private:
  ...
  struct 
  {
    uint32_t fUsesLocalCoords;
    uint32_t fUseGammaCorrectDistanceTable;
    uint32_t fNeedsGlyphTransform;
  };
  int fGeoCount;
  int fNumGlyphs;
  MaskType fMaskType;
