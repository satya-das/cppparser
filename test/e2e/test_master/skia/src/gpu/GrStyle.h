...
public:
  ...
  static SkScalar MatrixToScaleFactor(const SkMatrix& matrix)
  {
     ...
     * matrix.
     */
  bool SK_WARN_UNUSED_RESULT applyPathEffectToPath(SkPath* dst, SkStrokeRec* remainingStoke, const SkPath& src, SkScalar scale) const;
    /**
     ...
     */
  bool SK_WARN_UNUSED_RESULT applyToPath(SkPath* dst, SkStrokeRec::InitStyle* fillOrHairline, const SkPath& src, SkScalar scale) const;
...
private:
  ...
  struct DashInfo
  {
    ...
    DashInfo& operator=(const DashInfo& that)
    {
      ...
      sk_careful_memcpy(fIntervals.get(), that.fIntervals.get(), sizeof(SkScalar) * that.fIntervals.count());
      return *this;
    }
    void reset()
    {
