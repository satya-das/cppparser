...
namespace SK_OPTS_NS
{
  static void RGBA_to_rgbA_portable(uint32_t* dst, const uint32_t* src, int count)
  {
    for (int i = 0; i < count; i++)
    {
      ...
      g = (g * a + 127) / 255;
      r = (r * a + 127) / 255;
      dst[i] = (uint32_t) a << 24 | (uint32_t) b << 16 | (uint32_t) g << 8 | (uint32_t) r << 0;
    }
  }
  ...
  static void gray_to_RGB1_portable(uint32_t dst[], const uint8_t* src, int count)
  {
    for (int i = 0; i < count; i++)
    {
      dst[i] = (uint32_t) 0xFF << 24 | (uint32_t) src[i] << 16 | (uint32_t) src[i] << 8 | (uint32_t) src[i] << 0;
    }
  }
...
// Scale a byte by another, (x * y + 127) / 255
  ...
  static void expand_grayA(uint32_t dst[], const uint8_t* src, int count)
  {
    while (count >= 16)
    {
      ...
      if (kPremul)
      {
        ga.val[0] = vcombine_u8(scale(vget_low_u8(ga.val[0]), vget_low_u8(ga.val[1])), scale(vget_high_u8(ga.val[0]), vget_high_u8(ga.val[1])));
      }
        // Set each of the color channels.
      uint8x16x4_t rgba;
      rgba.val[0] = ga.val[0];
...
// Inputs are stored in 16-bit lanes, but are not larger than 8-bits.
  ...
  inline void grayA_to_RGBA(uint32_t dst[], const uint8_t* src, int count)
  {
    while (count >= 8)
    {
      ...
      __m128i gg = _mm_or_si128(_mm_and_si128(ga, _mm_set1_epi16(0x00FF)), _mm_slli_epi16(ga, 8));
