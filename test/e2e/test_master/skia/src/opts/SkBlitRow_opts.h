...
static __m256i SkPMSrcOver_AVX2(const __m256i& src, const __m256i& dst)
{
  ...
  const int _ = -1;
  __m256i srcA_x2 = _mm256_shuffle_epi8(src, _mm256_setr_epi8(3, _, 3, _, 7, _, 7, _, 11, _, 11, _, 15, _, 15, _, 3, _, 3, _, 7, _, 7, _, 11, _, 11, _, 15, _, 15, _));
  __m256i scale_x2 = _mm256_sub_epi16(_mm256_set1_epi16(256), srcA_x2);
...
{
  auto SkAlphaMulQ_SSE2 = ;
  return _mm_add_epi32(src, SkAlphaMulQ_SSE2(dst, _mm_sub_epi32(_mm_set1_epi32(256), _mm_srli_epi32(src, 24))));
}
#  endif
...
// y[i] are the i-th lanes of the corresponding NEON vectors.
  static uint8x8_t SkMulDiv255Round_neon8(uint8x8_t x, uint8x8_t y)
  {
...
// same convention.
  static uint8x8x4_t SkPMSrcOver_neon8(uint8x8x4_t dst, uint8x8x4_t src)
  {
