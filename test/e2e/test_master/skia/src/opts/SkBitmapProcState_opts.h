...
#  if  1 && SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_AVX2
  inline void S32_alpha_D32_filter_DX(const SkBitmapProcState& s, const uint32_t* xy, int count, uint32_t* colors)
  {
    ...
    if (count > 0)
    {
      __m256i active = skvx::bit_pun<__m256i>(count > skvx::Vec<8,int>{0, 1, 2, 3, 4, 5, 6, 7}), coords = _mm256_maskload_epi32((const int*) xy, active), pixels;
...
#  elif  1 && SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSSE3
  inline void S32_alpha_D32_filter_DX(const SkBitmapProcState& s, const uint32_t* xy, int count, uint32_t* colors)
  {
    ...
    int y0, y1, wy;
    decode_packed_coordinates_and_weight(*xy++, &y0, &y1, &wy);
    auto row0 = (const uint32_t*) ((const uint8_t*) s.fPixmap.addr() + y0 * s.fPixmap.rowBytes()), row1 = (const uint32_t*) ((const uint8_t*) s.fPixmap.addr() + y1 * s.fPixmap.rowBytes());
    while (count >= 4)
    {
      ...
      __m128i wr = _mm_shuffle_epi8(wx, _mm_setr_epi8(0, 0, 0, 0, 4, 4, 4, 4, 8, 8, 8, 8, 12, 12, 12, 12)), wl = _mm_sub_epi8(_mm_set1_epi8(16), wr);
            // We need to interlace wl and wr for _mm_maddubs_epi16().
      __m128i interlaced_x_weights_AB = _mm_unpacklo_epi8(wl, wr), interlaced_x_weights_CD = _mm_unpackhi_epi8(wl, wr);
      enum {
 A,B,C,D
      };
    ...
    while (count-- > 0)
    {
      ...
      __m128i wr = _mm_set1_epi8(wx), wl = _mm_sub_epi8(_mm_set1_epi8(16), wr);
...
#  elif  1 && SK_CPU_SSE_LEVEL >= SK_CPU_SSE_LEVEL_SSE2
  inline void S32_alpha_D32_filter_DX(const SkBitmapProcState& s, const uint32_t* xy, int count, uint32_t* colors)
  {
    ...
    int y0, y1, wy;
    decode_packed_coordinates_and_weight(*xy++, &y0, &y1, &wy);
    auto row0 = (const uint32_t*) ((const char*) s.fPixmap.addr() + y0 * s.fPixmap.rowBytes()), row1 = (const uint32_t*) ((const char*) s.fPixmap.addr() + y1 * s.fPixmap.rowBytes());
    ...
    while (count-- > 0)
    {
      ...
      decode_packed_coordinates_and_weight(*xy++, &x0, &x1, &wx);
            ...
            //    | tl  tr |
            //    | bl  br |
      const __m128i tl = _mm_cvtsi32_si128(row0[x0]), tr = _mm_cvtsi32_si128(row0[x1]), bl = _mm_cvtsi32_si128(row1[x0]), br = _mm_cvtsi32_si128(row1[x1]);
      ...
      __m128i L = _mm_unpacklo_epi8(_mm_unpacklo_epi32(bl, tl), _mm_setzero_si128()), R = _mm_unpacklo_epi8(_mm_unpacklo_epi32(br, tr), _mm_setzero_si128());
