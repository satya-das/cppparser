...
private:
     ...
     */
  typedef void (*RowProc) (void* dstRow, const uint8_t* src, int dstWidth, int bpp, int deltaSrc, int offset, const SkPMColor ctable[]);
  template <RowProc Proc>
  static void SkipLeading8888ZerosThen(void* dstRow, const uint8_t* src, int dstWidth, int bpp, int deltaSrc, int offset, const SkPMColor ctable[]);
  template <RowProc Proc>
  static void SkipLeadingGrayAlphaZerosThen(void* dst, const uint8_t* src, int width, int bpp, int deltaSrc, int offset, const SkPMColor ctable[]);
  ...
  const int fDstBPP;
  SkSwizzler(RowProc fastProc, RowProc proc, const SkPMColor* ctable, int srcOffset, int srcWidth, int dstOffset, int dstWidth, int srcBPP, int dstBPP);
  static std::unique_ptr<SkSwizzler> Make(const SkImageInfo& dstInfo, RowProc fastProc, RowProc proc, const SkPMColor* ctable, int srcBPP, int dstBPP, const SkCodec::Options& options, const SkIRect* frame);
